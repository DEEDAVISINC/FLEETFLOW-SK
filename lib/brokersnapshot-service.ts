...
class BrokerSnapshotService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.info('BrokerSnapshot service initialized');
    } catch (error) {
  ...
  async login(): Promise<boolean> {
    ...
    try {
      console.info('Navigating to BrokerSnapshot login page...');
      ...
      const loginButton = await this.page.$(
        'button[type="submit"], input[type="submit"], .login-btn, .btn-login'
      );
      if (loginButton) {
      ...
      const isDashboard =
        (await this.page.$(
          '.dashboard, .main-content, .user-menu, .logout'
        )) !== null;

      if (isDashboard) {
        this.isLoggedIn = true;
        console.info('Successfully logged into BrokerSnapshot');
        return true;
      } else {
  ...
  async searchCarrier(mcNumber: string): Promise<CarrierInfo | null> {
    ...
    try {
      console.info(`Searching for carrier MC-${mcNumber}...`);
      ...
      await this.page.waitForSelector(
        'input[name="mc"], input[name="mcNumber"], .search-input',
        { timeout: 10000 }
      );
      ...
      // Wait for results
      await this.page.waitForSelector(
        '.carrier-info, .search-results, .carrier-details',
        { timeout: 15000 }
      );
      ...
      console.info('Carrier information retrieved:', carrierInfo);
      return carrierInfo;
    } catch (error) {
  ...
  async searchDriver(
    ...
    try {
      console.info(
        `Searching for driver license ${licenseNumber} in ${state}...`
      );
      ...
      // Wait for search form
      await this.page.waitForSelector(
        'input[name="license"], input[name="licenseNumber"]',
        { timeout: 10000 }
      );
      ...
      // Wait for results
      await this.page.waitForSelector(
        '.driver-info, .search-results, .driver-details',
        { timeout: 15000 }
      );
      ...
      const driverInfo: DriverInfo = await this.page.evaluate(() => {
        ...
        return {
          ...
          endorsements: getTextContent('.endorsements')
            .split(',')
            .map((e) => e.trim())
            .filter((e) => e),
          medicalCertStatus: getTextContent('.medical-cert, .medical-status'),
          hazmatEndorsement: getTextContent('.hazmat, .endorsements')
            .toLowerCase()
            .includes('hazmat'),
        };
      });

      console.info('Driver information retrieved:', driverInfo);
      return driverInfo;
    } catch (error) {
  ...
  async searchShipper(
    ...
    try {
      console.info(
        `Searching for shipper: ${companyName}${state ? ' in ' + state : ''}...`
      );
      ...
      // Wait for search form
      await this.page.waitForSelector(
        'input[name="company"], input[name="companyName"], .shipper-search-input',
        { timeout: 10000 }
      );
      ...
      if (state) {
        const stateSelector =
          'select[name="state"], select[name="shipperState"]';
        const stateElement = await this.page.$(stateSelector);
        if (stateElement) {
          await this.page.select(stateSelector, state);
        }
      ...
      // Wait for results
      await this.page.waitForSelector(
        '.shipper-info, .search-results, .shipper-details, .company-info',
        { timeout: 15000 }
      );
      ...
      const shipperInfo: ShipperInfo = await this.page.evaluate(() => {
        ...
        return {
          companyName: getTextContent(
            '.company-name, .shipper-name, h1, h2, .business-name'
          ),
          address: getTextContent(
            '.address, .shipper-address, .street-address'
          ),
          ...
          contactPerson: getTextContent(
            '.contact-person, .contact-name, .primary-contact'
          ),
          businessType: getTextContent('.business-type, .industry, .category'),
          yearEstablished: getTextContent(
            '.established, .year-established, .founded'
          ),
          creditRating: getTextContent(
            '.credit-rating, .rating, .credit-score'
          ),
          paymentTerms: getTextContent('.payment-terms, .terms'),
          averageLoadValue: parseNumber(
            getTextContent('.avg-load-value, .average-value')
          ),
      ...
      console.info('Shipper information retrieved:', shipperInfo);
      return shipperInfo;
    } catch (error) {
  ...
  async close(): Promise<void> {
    if (this.browser) {
      ...
      console.info('BrokerSnapshot service closed');
    }
  }
}

export default BrokerSnapshotService;
export type { BrokerSnapshotCredentials, CarrierInfo, DriverInfo, ShipperInfo };
