...
import { workflowIntegrationService } from '../app/services/WorkflowIntegrationService';
import { EDI204Data, EDI214Data, ediService } from './ediService';
...
class WorkflowManager {
  ...
  /**
   * Initialize a new load workflow with configurable photo requirements
   */
  initializeLoadWorkflow(
    loadId: string,
    driverId: string,
    dispatcherId: string,
    photoConfig?: {
    ...
    const config = { ...defaultPhotoConfig, ...photoConfig };
      ...
      steps: [
        {
          ...
          description:
            'Driver must confirm receipt and acceptance of load assignment',
          required: true,
          completed: false,
          allowOverride: false,
        },
        {
          ...
          description: 'Driver reviews rate confirmation and load details',
          required: true,
          completed: false,
          allowOverride: false,
        },
        {
          ...
          description: 'Driver verifies and accepts rate confirmation terms',
          required: true,
          completed: false,
          allowOverride: false,
        },
        {
          ...
          name: 'BOL Generation',
          description:
            'Bill of Lading is automatically generated after rate confirmation',
          required: true,
          completed: false,
          allowOverride: false,
          data: {
        ...
        },
        {
          ...
          description:
            'Driver confirms receipt of Bill of Lading documentation',
          required: true,
          completed: false,
          allowOverride: false,
        },
        {
          ...
          description: 'Driver verifies BOL details match load assignment',
          required: true,
          completed: false,
          allowOverride: false,
        },
        {
          ...
          description:
            'Driver receives authorization to proceed to pickup location',
          required: true,
          completed: false,
          allowOverride: false,
        },
        {
          id: 'pickup_arrival',
          name: 'Pickup Arrival',
          description: 'Driver arrives at pickup location and checks in',
          required: true,
          completed: false,
          allowOverride: false,
        },
        {
          ...
          description: config.pickupPhotosRequired
            ? `Driver completes pickup with ${config.canSkipPhotos ? 'optional' : 'REQUIRED'} photos, signature, and documentation`
            : 'Driver completes pickup with signature and documentation',
          required: true,
          completed: false,
          allowOverride: true,
          data: {
            requiresPhotos: config.pickupPhotosRequired,
            minimumPhotos: config.minimumPhotos,
            photoTypes: config.photoTypes?.filter((type) =>
              ['loaded_truck', 'bill_of_lading'].includes(type)
            ) || ['loaded_truck', 'bill_of_lading'],
            canSkipPhotos: config.canSkipPhotos, // Only photos can be skipped, other validations remain mandatory
          },
        },
        {
          id: 'transit_start',
        ...
        },
        {
          ...
          description:
            'Real-time location tracking and status updates during transit',
          required: true,
          completed: false,
          allowOverride: false,
          data: {
        ...
        },
        {
          ...
          description: config.deliveryPhotosRequired
            ? `Driver completes delivery with ${config.canSkipPhotos ? 'optional' : 'REQUIRED'} photos, BOL signing, receiver signature, and receiver info`
            : 'Driver completes delivery with BOL signing, receiver signature, and receiver info',
          required: true,
          completed: false,
          allowOverride: true,
          data: {
            requiresPhotos: config.deliveryPhotosRequired,
            minimumPhotos: config.minimumPhotos,
            photoTypes: config.photoTypes?.filter((type) =>
              ['unloaded_truck', 'delivery_receipt'].includes(type)
            ) || ['unloaded_truck', 'delivery_receipt'],
            canSkipPhotos: config.canSkipPhotos, // Only photos can be skipped, other validations remain mandatory
            requiresReceiverName: true,
            requiresReceiverSignature: true,
            // 🚨 CRITICAL: BOL signing requirements added
            requiresBOLSigning: true,
            requiresReceiverBOLSignature: true,
            bolSigningSequence: [
              'photos',
              'bol_signing',
              'receiver_info',
            ...
            generateSignedDocument: true,
            triggerWorkflowCascade: true,
          },
        },
        {
          id: 'pod_submission',
          name: 'POD Submission',
          description: 'Driver submits proof of delivery documentation',
          required: true,
          completed: false,
          allowOverride: false,
        },
  ...
  async completeStep(
    ...
    // Persist to backend
    try {
      await workflowBackendService.completeStep(
        loadId,
        stepId,
        completedBy || 'system',
        data
    ...
    // 🚨 CRITICAL: Trigger workflow integration cascade for connected processes
    try {
      ...
      console.error('Error triggering workflow integration cascade:', error);
      // Don't fail the step completion if integration cascade fails
    }
   ...
   */
  async initializeWorkflowFromBackend(
    loadId: string,
    driverId: string,
    dispatcherId: string
  ): Promise<LoadWorkflow> {
    try {
      ...
      if (!workflow) {
        ...
        const workflowId = await workflowBackendService.createWorkflow(
          loadId,
          driverId,
          dispatcherId
        );
  ...
  async uploadStepDocument(
    ...
    try {
      await workflowBackendService.uploadStepDocument(
        loadId,
        stepId,
        fileUrl,
        fileType,
  ...
  async requestOverride(
    ...
    try {
      ...
      // Persist to backend
      await workflowBackendService.requestOverride(
        loadId,
        stepId,
        reason,
        requestedBy
  ...
  getAvailableSteps(loadId: string): WorkflowStep[] {
    ...
    for (const step of workflow.steps) {
      ...
      const canComplete = this.canCompleteStep(
        loadId,
   ...
   */
  private async triggerStepNotifications(
    loadId: string,
    stepId: WorkflowStepId,
    ...
    const notifications = {
      load_assignment_confirmation: async () => {
        this.notifyDispatcher(
          workflow.dispatcherId,
          'Driver confirmed load assignment',
          loadId
        );
      ...
      rate_confirmation_review: () =>
        this.notifyDispatcher(
          workflow.dispatcherId,
          'Driver reviewed rate confirmation',
          loadId
        ),
      rate_confirmation_verification: async () => {
        this.notifyDispatcher(
          workflow.dispatcherId,
          'Driver verified rate confirmation',
          loadId
        );
      ...
      bol_receipt_confirmation: () =>
        this.notifyDispatcher(
          workflow.dispatcherId,
          'Driver confirmed BOL receipt',
          loadId
        ),
      bol_generation: async () => {
        this.notifyDispatcher(
          workflow.dispatcherId,
          'BOL generated and ready for driver pickup',
          loadId
        );
        // EDI: Generate EDI 211 (Motor Carrier BOL) if required
        await this.triggerEDINotification(loadId, stepId, 'bol_generated');
      },
      bol_verification: () =>
        this.notifyDispatcher(
          workflow.dispatcherId,
          'Driver verified BOL - pickup authorized',
          loadId
        ),
      pickup_authorization: () =>
        this.notifyDispatcher(
          workflow.dispatcherId,
          'Driver authorized for pickup',
          loadId
        ),
      ...
      pickup_completion: async () => {
        this.notifyDispatcher(
          workflow.dispatcherId,
          'Driver completed pickup with required photos',
          loadId
        );
      ...
      transit_start: async () => {
        this.notifyDispatcher(
          workflow.dispatcherId,
          'Driver started transit to delivery',
          loadId
        );
      ...
      transit_tracking: async () => {
        this.notifyDispatcher(
          workflow.dispatcherId,
          'Real-time tracking enabled for transit',
          loadId
        );
      ...
      delivery_completion: async () => {
        this.notifyDispatcher(
          workflow.dispatcherId,
          'Driver completed delivery with required photos and receiver signature',
          loadId
        );
   ...
   */
  private async triggerEDINotification(
    loadId: string,
    stepId: WorkflowStepId,
    eventType: string
  ): Promise<void> {
        ...
        switch (eventType) {
          ...
          case 'rate_confirmed':
            if (partner.supportedTransactions.includes('204')) {
              ...
              const message = await ediService.generateEDI204(
                edi204Data,
                partnerId
              );
          ...
          case 'delivered':
            if (partner.supportedTransactions.includes('214')) {
              ...
              const statusMapping = {
                ...
                pickup_completed: {
                  code: 'AF',
                  description: 'Departed from pickup location',
                },
                in_transit: {
                  code: 'X1',
                  description: 'In transit to delivery',
                },
                delivery_arrival: {
                  code: 'A2',
              ...
              const status =
                statusMapping[eventType as keyof typeof statusMapping];
              const edi214Data: EDI214Data = {
                shipmentId: loadData.loadId,
                statusCode: status.code,
                statusDescription: status.description,
                location: eventType.includes('delivery')
              ...
              const message = await ediService.generateEDI214(
                edi214Data,
                partnerId
              );
              await ediService.sendEDI(message.id);
  ...
  validateStepData(
    ...
    const validations = {
      ...
      rate_confirmation_review: (data: any) => {
        const errors = [];
        if (!data.documentReviewed)
          errors.push('Rate confirmation document must be reviewed');
        return { valid: errors.length === 0, errors };
      },
      rate_confirmation_verification: (data: any) => {
        const errors = [];
        if (!data.rateAccepted)
          errors.push('Rate acceptance confirmation required');
        if (!data.verificationTimestamp)
          errors.push('Verification timestamp required');
        return { valid: errors.length === 0, errors };
      },
      ...
      bol_generation: (data: any) => {
        const errors = [];
        if (!data.bolGenerated) errors.push('BOL generation must be completed');
        if (!data.bolNumber) errors.push('BOL number required');
        return { valid: errors.length === 0, errors };
      },
      ...
      pickup_authorization: (data: any) => {
        const errors = [];
        if (!data.readyToDepart)
          errors.push('Ready to depart confirmation required');
        return { valid: errors.length === 0, errors };
      },
      ...
      pickup_completion: (data: any) => {
        ...
        if (!data.loadingComplete)
          errors.push('Loading completion confirmation required');

        // REQUIRED: Photos at pickup
        if (!data.pickupPhotos || data.pickupPhotos.length < 2) {
          errors.push(
            'Minimum 2 photos required at pickup (loaded truck & BOL)'
          );
        }
      ...
      transit_tracking: (data: any) => {
        ...
        if (!data.locationUpdateInterval)
          errors.push('Location update interval required');
        if (!data.statusUpdateEnabled)
          errors.push('Status update must be enabled');
        return { valid: errors.length === 0, errors };
      },
      ...
      delivery_completion: (data: any) => {
        ...
        if (!data.unloadingComplete)
          errors.push('Unloading completion confirmation required');

        // REQUIRED: Photos at delivery
        if (!data.deliveryPhotos || data.deliveryPhotos.length < 2) {
          errors.push(
            'Minimum 2 photos required at delivery (unloaded truck & delivery receipt)'
          );
        }
        ...
        if (!data.receiverSignature && !data.overrideApproved) {
          errors.push(
            'Receiver signature required (or override approval needed)'
          );
        }
        ...
        if (data.overrideApproved && !data.overrideReason) {
          errors.push(
            'Override reason required when signature/name override is used'
          );
        }
      ...
      pod_submission: (data: any) => {
        const errors = [];
        if (!data.allPhotosUploaded)
          errors.push('All required photos must be uploaded');
        if (!data.documentationComplete)
          errors.push('Documentation completion confirmation required');
        return { valid: errors.length === 0, errors };
      },
    };
