...
import { GoogleMap, LoadScript, Marker, Polyline, InfoWindow, TrafficLayer, TransitLayer } from '@react-google-maps/api'
  ...
  loading: () => (
    <div style={{
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ fontSize: '14px', opacity: 0.8, marginTop: '8px' }}>Real-time tracking data</div>
      </div>
    </div>
  )
})
...
export default function LiveTrackingPage() {
  ...
  const [shipments, setShipments] = useState<Shipment[]>([
    ...
    },
    {
      ...
      progress: 100,
      currentLocation: [33.7490, -84.3880],
  ...
  const [sortBy, setSortBy] = useState<'id' | 'status' | 'eta' | 'progress' | 'created' | 'miles'>('id')
  ...
  const handleExportCSV = () => {
    const shipmentsToExport = selectedShipments.size > 0 
      ? filteredShipments.filter(s => selectedShipments.has(s.id))
      : filteredShipments

    const csvHeaders = [
      'ID', 'Status', 'Origin', 'Destination', 'Carrier', 'Progress', 'Speed', 'ETA', 
      'Driver', 'Phone', 'Vehicle', 'Priority', 'Value', 'Weight', 'Commodity', 
      'Customer', 'Created', 'Pickup', 'Delivery', 'Miles'
    ]
    ...
    link.setAttribute('download', `fleetflow-shipments-${new Date().toISOString().split('T')[0]}.csv`)
  ...
  const handleBulkStatusUpdate = (newStatus: string) => {
    ...
    const validStatuses = ['in-transit', 'delivered', 'delayed', 'loading', 'unloading']
  ...
  const filteredShipments = useMemo(() => {
    return shipments.filter(shipment => {
      ...
      // Priority filter
      if (selectedPriority.length > 0 && !selectedPriority.includes(shipment.priority)) {
        return false
      }
      ...
      // Value range filter
      if (valueRange) {
        if (shipment.value < valueRange.min || shipment.value > valueRange.max) {
          return false
        }
      }

      // Carrier filter
      if (selectedCarriers.length > 0 && !selectedCarriers.includes(shipment.carrier)) {
        return false
      }
  ...
  }, [shipments, filterStatus, selectedPriority, dateRange, searchTerm, valueRange, selectedCarriers])
  ...
  const stats = useMemo(() => {
    ...
    const avgProgress = shipments.reduce((sum, s) => sum + s.progress, 0) / totalShipments
          ...
          <div>
            <h1 style={{
                ...
                <p style={{
                  ...
                  Real-time shipment monitoring & intelligent logistics management
                </p>
                <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
            ...
            <button
              ...
              style={{
                ...
                color: activeView === tab.id
                  ...
                  : 'white',
                backdropFilter: 'blur(10px)',
                border: activeView === tab.id ? '1px solid rgba(255, 255, 255, 0.4)' : '1px solid rgba(255, 255, 255, 0.2)',
                transform: activeView === tab.id ? 'translateY(-2px)' : 'translateY(0)',
                boxShadow: activeView === tab.id ? '0 8px 25px rgba(0, 0, 0, 0.2)' : 'none'
              }}
            >
        ...
        <div style={{
          ...
          <div style={{
            ...
            alignItems: 'center',
                ...
                borderRadius: '8px',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '32px' }}>
                      ...
                      <div style={{ textAlign: 'center' }}>
                        <p style={{ color: getStatusColor(shipment.status), fontSize: '14px', fontWeight: '600', margin: '0 0 4px 0' }}>
                      ...
                      <div style={{ textAlign: 'center' }}>
                        <p style={{ color: 'white', fontSize: '14px', fontWeight: '600', margin: '0 0 4px 0' }}>
            ...
            <div style={{
              ...
              <h2 style={{ color: 'white', fontSize: '24px', fontWeight: 'bold', margin: '0 0 24px 0' }}>
                Quick Actions
              </h2>
              <div style={{
                display: 'grid',
        ...
        <div style={{
            ...
            <div style={{ padding: '32px' }}>
              ...
              <div style={{ marginBottom: '24px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                  <span style={{ color: 'white', fontSize: '14px', fontWeight: '500' }}>Filter Presets:</span>
          ...
          {/* Search and Filters */}
            ...
            <input
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <input
                          ...
                          onChange={(e) => setValueRange(prev => ({ ...prev, min: parseInt(e.target.value) || 0, max: prev?.max || 1000000 }))}
                          style={{
                            flex: 1,
                            padding: '8px',
                            borderRadius: '4px',
                        ...
                        <input
                          ...
                          onChange={(e) => setValueRange(prev => ({ ...prev, max: parseInt(e.target.value) || 1000000, min: prev?.min || 0 }))}
                          style={{
                            flex: 1,
                            padding: '8px',
                            borderRadius: '4px',
              ...
              {selectedShipments.size > 0 && (
                <div style={{
                  ...
                  <span style={{ color: 'white', fontSize: '14px' }}>
                    {selectedShipments.size} of {filteredShipments.length} selected
                  </span>
                  <div style={{ display: 'flex', gap: '12px' }}>
                    <button
                      onClick={handleExportCSV}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: '24px', minHeight: '600px' }}>
                {/* Map */}
                <div style={{
                  background: 'rgba(255, 255, 255, 0.05)',
                  borderRadius: '12px',
                  ...
                  <MapComponent 
                    ...
                    onSelectShipment={(shipmentId) => {
                      const shipment = shipmentId ? sortedShipments.find(s => s.id === shipmentId) : null
          ...
          {/* Shipment List */}
                <div style={{
                  ...
                  <div style={{
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'rgba(255, 255, 255, 0.8)', fontSize: '12px' }}>
                      <input
                        type="checkbox"
            ...
            <div style={{ padding: '32px' }}>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <h3 style={{ color: 'white', fontSize: '24px', fontWeight: 'bold', margin: '0 0 4px 0' }}>
              ...
              <div style={{
                ...
                <h3 style={{ color: 'white', fontSize: '20px', fontWeight: 'bold', margin: '0 0 16px 0' }}>
                  Status Distribution
                </h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '16px' }}>
                  {[
                    { status: 'in-transit', count: stats.inTransit, color: '#3b82f6' },
                    { status: 'delivered', count: stats.delivered, color: '#10b981' },
                    { status: 'delayed', count: stats.delayed, color: '#ef4444' },
                    { status: 'loading', count: shipments.filter(s => s.status === 'loading').length, color: '#f59e0b' },
                    { status: 'unloading', count: shipments.filter(s => s.status === 'unloading').length, color: '#8b5cf6' }
                  ].map((item) => (
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {Array.from(new Set(shipments.map(s => s.carrier))).map(carrier => {
                    const carrierShipments = shipments.filter(s => s.carrier === carrier)
                    const avgProgress = carrierShipments.reduce((sum, s) => sum + s.progress, 0) / carrierShipments.length
                    const onTimeCount = carrierShipments.filter(s => s.status === 'delivered').length
                    const delayedCount = carrierShipments.filter(s => s.status === 'delayed').length
                    const onTimeRate = onTimeCount / (onTimeCount + delayedCount) * 100 || 0
...
function EnhancedSummaryCard({ label, value, color, icon }: { label: string, value: string | number, color: string, icon: string }) {
  return (
    <div style={{
...
function ShipmentCard({ 
  ...
  return (
    <div
      onClick={onSelect}
      style={{
        background: isSelected ? 'rgba(59, 130, 246, 0.15)' : 'rgba(255, 255, 255, 0.95)',
        border: isSelected ? '1px solid rgba(59, 130, 246, 0.4)' : '1px solid rgba(148, 163, 184, 0.3)',
        borderRadius: '12px',
        padding: '16px',
      ...
      <div style={{ marginBottom: '12px' }}>
        <div style={{ 
          ...
          <span style={{ fontSize: '0.625rem', color: '#475569' }}>Progress</span>
          <span style={{ fontSize: '0.625rem', color: '#0f172a', fontWeight: 600 }}>{shipment.progress}%</span>
        </div>
                  <div style={{
          width: '100%',
          height: '4px',
