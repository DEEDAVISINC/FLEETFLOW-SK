...
const BrokerOperationsPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'overview' | 'rfx' | 'shippers' | 'quotes' | 'loadboard' | 'tools' | 'analytics'>('overview');
  const [user] = useState({
    name: 'Alex Rodriguez',
    role: 'Senior Broker',
  ...
  const createLoad = async () => {
    ...
    try {
      // Create load in broker board using the new flow system
      const loadData = {
        brokerId: user.id,
      ...
      const createdLoad = createLoadInBrokerBoard(loadData);
      ...
      alert(`Load ${createdLoad.loadBoardNumber} created successfully in broker board!`);
    } catch (error) {
  ...
  const assignDispatcher = (loadId: string, dispatcherId: string) => {
    try {
      const assignedLoad = assignLoadToDispatcher(loadId, dispatcherId);
      if (assignedLoad) {
        // Update local state
        setLoads(loads.map(load =>
          load.id === loadId ? assignedLoad : load
        ));

        // Update dispatcher stats
        setDispatchers(dispatchers.map(d =>
          d.id === dispatcherId
            ? { ...d, activeLoads: d.activeLoads + 1 }
            : d
        ));

        alert(`Load ${assignedLoad.loadBoardNumber} assigned to dispatcher successfully!`);
      } else {
        alert('Error assigning load to dispatcher.');
      }
    } catch (error) {
              ...
              <div style={{
                ...
                <div style={{ display: 'grid', gap: '12px' }}>
                  <div style={{
                    ...
                    <div style={{ color: 'white', fontWeight: '600', marginBottom: '8px', fontSize: '16px' }}>
                      📦 Freight Class for RFx
                    </div>
                    <div style={{ color: 'rgba(255, 255, 255, 0.8)', fontSize: '14px', marginBottom: '12px' }}>
                      Automatically determine freight class for accurate RFQ, RFB, RFP, and RFI responses
                    </div>
                    <div style={{ color: 'rgba(255, 255, 255, 0.6)', fontSize: '12px' }}>
                      ✓ Integrated with RFx Response System<br/>
                      ✓ NMFC database lookup<br/>
                  ...
                  <div style={{
                    ...
                    <div style={{ color: 'rgba(255, 255, 255, 0.8)', fontSize: '14px', marginBottom: '12px' }}>
                      Enhance quotes with accurate freight classification for competitive bidding
                    </div>
                    <div style={{ color: 'rgba(255, 255, 255, 0.6)', fontSize: '12px' }}>
                      ✓ Connected to Broker Box<br/>
                      ✓ Integrated quoting workflow<br/>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px', marginBottom: '20px' }}>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setNewLoad({...newLoad, type: e.target.value as 'LTL' | 'FTL' | 'Specialized'})}
                      style={{
                        width: '100%',
                        padding: '12px',
                        borderRadius: '8px',
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={`${newLoad.origin?.city || ''}, ${newLoad.origin?.state || ''} ${newLoad.origin?.zipCode || ''}`.trim().replace(/^,\s*/, '')}
                      onChange={(e) => {
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="City, State, ZIP"
                      value={`${newLoad.destination?.city || ''}, ${newLoad.destination?.state || ''} ${newLoad.destination?.zipCode || ''}`.trim().replace(/^,\s*/, '')}
                      onChange={(e) => {
                        const parts = e.target.value.split(',');
                        setNewLoad({
                          ...newLoad,
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="1000"
                      value={newLoad.rate || ''}
                      onChange={(e) => setNewLoad({...newLoad, rate: parseFloat(e.target.value)})}
                      style={{
                        width: '100%',
                        padding: '12px',
                        borderRadius: '8px',
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '20px' }}>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setNewLoad({...newLoad, weight: parseFloat(e.target.value)})}
                        style={{
                          width: '100%',
                          padding: '12px',
                          borderRadius: '8px',
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setNewLoad({...newLoad, pallets: parseInt(e.target.value)})}
                        style={{
                          width: '100%',
                          padding: '12px',
                          borderRadius: '8px',
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <select
                          ...
                          onChange={(e) => setNewLoad({...newLoad, freightClass: parseFloat(e.target.value)})}
                          style={{
                            flex: 1,
                            padding: '12px',
                            borderRadius: '8px',
                        ...
                        <button
                          type="button"
                          onClick={() => setSelectedLoad({...newLoad, id: 'NEW'} as LoadPosting)}
                          style={{
                            padding: '12px 16px',
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewLoad({...newLoad, commodity: e.target.value})}
                      style={{
                        width: '100%',
                        padding: '12px',
                        borderRadius: '8px',
            ...
            <div style={{
              ...
              {loads.length === 0 ? (
                <div style={{ textAlign: 'center', color: 'rgba(255, 255, 255, 0.7)', padding: '40px' }}>
                  ...
                  <div>No loads posted yet. Create your first load posting above.</div>
                </div>
              ) : (
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ color: 'white', margin: 0, fontSize: '18px' }}>
                Freight Class Calculator {selectedLoad.id !== 'NEW' ? `for Load ${selectedLoad.id}` : 'for New Load'}
              </h3>
              <button
