...
export const DocumentUploadEnhanced: React.FC<DocumentUploadProps> = ({ onDocumentUploaded, onNext, onBack, carrierData }) => {
  ...
  const requiredDocsCompleted = documents.filter(doc => doc.required && doc.status === 'approved').length;
  const totalRequiredDocs = documents.filter(doc => doc.required).length;
  const completionPercentage = Math.round((requiredDocsCompleted / totalRequiredDocs) * 100);
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: '32px' }}>
        ...
        <p style={{ color: 'rgba(255, 255, 255, 0.8)', fontSize: '1.1rem', marginBottom: '20px' }}>
          Upload required documents for automated verification and instant processing
        </p>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            <span style={{ color: 'white', fontWeight: 'bold' }}>Document Verification Progress</span>
            <span style={{ color: 'white', fontWeight: 'bold' }}>
              {requiredDocsCompleted}/{totalRequiredDocs} ({completionPercentage}%)
            </span>
          </div>
          <div style={{
        ...
        {filteredDocuments.map((document) => (
          <div
            ...
            style={{
              ...
              padding: '20px',
              border: `2px solid ${
                document.status === 'approved' ? 'rgba(16, 185, 129, 0.5)' :
                document.status === 'rejected' || document.status === 'needs_attention' ? 'rgba(239, 68, 68, 0.5)' :
                document.required && !document.uploaded ? 'rgba(239, 68, 68, 0.5)' : 
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '12px' }}>
              ...
              <div style={{
                ...
                {getStatusIcon(document.status)} {document.status.toUpperCase().replace('_', ' ')}
              </div>
            </div>
            ...
            {document.uploaded ? (
              <div style={{
                background: document.status === 'approved' 
                  ? 'rgba(16, 185, 129, 0.2)' 
                  : document.status === 'rejected' || document.status === 'needs_attention'
                    ? 'rgba(239, 68, 68, 0.2)'
                    : 'rgba(59, 130, 246, 0.2)',
                border: `1px solid ${
                ...
                <div style={{ color: 'white', fontSize: '0.9rem' }}>
                  <div style={{ marginBottom: '4px' }}><strong>File:</strong> {document.fileName}</div>
                  <div style={{ marginBottom: '4px' }}><strong>Uploaded:</strong> {document.uploadDate}</div>
                  {document.expirationDate && (
                    <div style={{ marginBottom: '4px' }}><strong>Expires:</strong> {document.expirationDate}</div>
                  )}
                  {document.verificationResult && (
                    <div style={{ marginBottom: '4px' }}>
                      <strong>Confidence:</strong> {Math.round(document.verificationResult.confidence * 100)}%
                    </div>
                  )}
                </div>
                  ...
                  <div style={{ marginTop: '8px' }}>
                    ...
                    {document.status === 'needs_attention' && (
                      <div style={{ 
                        ...
                        📞 Manual review required - Our team will contact you within 2 business hours
                      </div>
                    )}
                  </div>
                )}
              ...
              <div style={{ marginTop: '12px' }}>
                ...
                {document.id === 'w9_form' ? (
                  ...
                  <div>
                    ...
                    >
                      {uploadingId === document.id ? '⏳ Processing...' : '📤 Choose File to Upload'}
                    </label>
                  </div>
                )}
              </div>
      ...
      <div style={{
        ...
        {completionPercentage === 100 ? (
          <div>
            ...
            <h3 style={{ color: '#10b981', fontSize: '1.4rem', fontWeight: 'bold', marginBottom: '8px' }}>
              All Documents Verified!
            </h3>
            <p style={{ color: 'rgba(255, 255, 255, 0.8)', marginBottom: '16px' }}>
              Your onboarding will be completed within 24 hours. You will receive portal access credentials via email.
            </p>
            <div style={{
          ...
          <div>
            <h3 style={{ color: 'white', fontSize: '1.2rem', fontWeight: 'bold', marginBottom: '8px' }}>
              {totalRequiredDocs - requiredDocsCompleted} Required Documents Remaining
            </h3>
