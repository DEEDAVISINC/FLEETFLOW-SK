...
import ContractorOnboardingService, { OnboardingSession, OnboardingStep, DocumentRequirement } from '../../services/ContractorOnboardingService'
...
const ContractorOnboardingDashboard = () => {
  ...
  const loadOnboardingSession = async () => {
    try {
      ...
      const sessionData = await ContractorOnboardingService.getOnboardingSession(sessionId)
  ...
  const handleBackgroundCheck = async () => {
    ...
    const result = await ContractorOnboardingService.initiateBackgroundCheck(contractorData)
  ...
  const getNextAction = (step: OnboardingStep) => {
    const actionMap: Record<string, { text: string; action: string }> = {
      'background_check': { text: 'Complete Background Check', action: 'complete_background' },
      'contract_signing': { text: 'Review & Sign Contract', action: 'review_contract' },
      'nda_signing': { text: 'Review & Sign NDA', action: 'review_nda' },
      'insurance_verification': { text: 'Upload Insurance', action: 'upload_insurance' },
      'training_completion': { text: 'Complete Training', action: 'complete_training' }
    }

    return actionMap[step.id] || { text: 'Complete Step', action: 'complete_step' }
  }
  ...
  if (loading) {
    return (
      <div style={{
        ...
        <div style={{ color: 'white', fontSize: '18px' }}>Loading onboarding dashboard...</div>
      </div>
    )
  }
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#0369a1' }}>
                {displaySession.steps.filter(s => s.status === 'completed').length}
              </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#d97706' }}>
                {displaySession.steps.filter(s => s.status === 'in_progress').length}
              </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#4b5563' }}>
                {displaySession.steps.filter(s => s.status === 'pending').length}
              </div>
                ...
                return (
                  ...
                  >
                    ...
                    {step.status === 'in_progress' && (
                      <button
                        onClick={() => handleStepAction(step.id, nextAction.action)}
                        ...
                        style={{
                          ...
                          fontSize: '14px',
                          fontWeight: '500',
                          cursor: processingStep === step.id ? 'not-allowed' : 'pointer',
                          opacity: processingStep === step.id ? 0.5 : 1
                        }}
                      >
                        {processingStep === step.id ? 'Processing...' : nextAction.text}
                      </button>
                    )}
                  </div>
                )
