    ...
    <div style={{
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          {[
            { label: 'Available Loads', value: filteredLoads.length, color: 'linear-gradient(135deg, #10b981, #059669)', icon: '📋' },
            { label: 'Avg Rate', value: `$${filteredLoads.length > 0 ? Math.round(filteredLoads.reduce((sum, load) => sum + load.rate, 0) / filteredLoads.length).toLocaleString() : '0'}`, color: 'linear-gradient(135deg, #3b82f6, #2563eb)', icon: '💰' },
            { label: 'Top Rate', value: `$${Math.max(...filteredLoads.map(load => load.rate), 0).toLocaleString()}`, color: 'linear-gradient(135deg, #f97316, #ea580c)', icon: '🎯' },
            { label: 'Support', value: '24/7', color: 'linear-gradient(135deg, #a855f7, #9333ea)', icon: '🕐' }
          ].map((stat, index) => (
            <div key={index} style={{
            ...
            onMouseOut={(e) => {
              ...
              <div style={{
                ...
                <div style={{ fontSize: '14px', opacity: 0.9, marginBottom: '8px' }}>{stat.icon}</div>
                <div>{stat.value}</div>
              </div>
              <div style={{ color: '#374151', fontSize: '14px', fontWeight: '600' }}>{stat.label}</div>
            </div>
          ))}
        </div>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead style={{ background: 'linear-gradient(135deg, #f9fafb, #f3f4f6)' }}>
                  <tr>
                    <th style={{ padding: '16px 24px', textAlign: 'left', fontSize: '12px', fontWeight: 'bold', color: '#374151', textTransform: 'uppercase', letterSpacing: '0.05em' }}>📞 Board #</th>
                    ...
                    try {
                      return filteredLoads.map((load, index) => (
                        ...
                        onMouseOut={(e) => {
                          ...
                          <td style={{ padding: '16px 24px', fontSize: '14px', fontWeight: 'bold', color: '#059669', fontFamily: 'monospace' }}>
