...
const verifyWebhookAuth = (request: NextRequest): boolean => {
  ...
  const expectedSecret = process.env.WEBHOOK_SECRET || 'fleetflow-webhook-secret'
...
const triggerExternalIntegrations = async (
  ...
  // ELD integration
  if (payload.new_status === 'picked_up' || payload.new_status === 'delivered') {
...
const updateELDSystem = async (payload: LoadStatusWebhookPayload, load: any) => {
  // Implementation for ELD system update
  console.log('🚛 Updating ELD system with status change')
}

const triggerInvoicing = async (payload: LoadStatusWebhookPayload, load: any) => {
  // Implementation for automatic invoicing
  console.log('💰 Triggering automatic invoicing for completed load')
}

const sendPushNotifications = async (notifications: any[], payload: LoadStatusWebhookPayload) => {
