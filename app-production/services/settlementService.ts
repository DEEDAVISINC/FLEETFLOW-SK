...
const saveToStorage = () => {
  if (typeof window !== 'undefined') {
    localStorage.setItem('fleetflow_settlements', JSON.stringify(SETTLEMENTS_DB));
    localStorage.setItem('fleetflow_profitability', JSON.stringify(PROFITABILITY_DB));
  }
};

// Settlement Management Functions

export const createSettlement = (settlementData: Omit<SettlementData, 'id' | 'createdDate'>): SettlementData => {
...
export const getSettlements = (filters?: {
  ...
  if (filters) {
    ...
    if (filters.periodStart) {
      settlements = settlements.filter(s => s.periodStart >= filters.periodStart!);
    }
    if (filters.periodEnd) {
      settlements = settlements.filter(s => s.periodEnd <= filters.periodEnd!);
    }
  }
  
  return settlements.sort((a, b) => new Date(b.createdDate).getTime() - new Date(a.createdDate).getTime());
};
...
export const updateSettlementStatus = (
  ...
  const settlementIndex = SETTLEMENTS_DB.findIndex(s => s.id === settlementId);
  ...
  SETTLEMENTS_DB[settlementIndex] = { ...SETTLEMENTS_DB[settlementIndex], ...updates };
  saveToStorage();
...
export const calculateLoadProfitability = (loadData: {
  ...
  const { customerRate, carrierPay, brokerCommission = 0, dispatchFee = 0, expenses, miles } = loadData;
  
  const totalExpenses = Object.values(expenses).reduce((sum, exp) => sum + exp, 0);
  const totalCosts = carrierPay + brokerCommission + dispatchFee + totalExpenses;
...
export const getProfitabilityData = (filters?: {
  ...
  return data.sort((a, b) => new Date(b.dates.pickupDate).getTime() - new Date(a.dates.pickupDate).getTime());
};
...
export const calculateFinancialMetrics = (
  ...
  let filteredData = PROFITABILITY_DB.filter(
    d => d.dates.pickupDate >= dateRange.start && d.dates.pickupDate <= dateRange.end
  );
  ...
  const totalRevenue = filteredData.reduce((sum, d) => sum + d.financial.totalRevenue, 0);
  const totalCosts = filteredData.reduce((sum, d) => sum + d.financial.totalCosts, 0);
  const grossProfit = filteredData.reduce((sum, d) => sum + d.financial.grossProfit, 0);
  const netProfit = filteredData.reduce((sum, d) => sum + d.financial.netProfit, 0);
  ...
  const metrics: FinancialMetrics = {
    ...
    expenses: {
      ...
      byCategory: {
        ...
        maintenance: filteredData.reduce((sum, d) => sum + d.financial.maintenanceCosts, 0),
        tolls: filteredData.reduce((sum, d) => sum + d.financial.tollCosts, 0),
        permits: filteredData.reduce((sum, d) => sum + d.financial.permitCosts, 0),
        insurance: 0, // Would be calculated from separate insurance data
        equipment: 0, // Would be calculated from separate equipment data
        other: filteredData.reduce((sum, d) => sum + d.financial.otherExpenses, 0)
      },
      byVehicle: {}
    },
    profitability: {
      ...
      avgProfitPerLoad: filteredData.length > 0 ? netProfit / filteredData.length : 0,
      avgMarginPerLoad: filteredData.length > 0 ? 
        filteredData.reduce((sum, d) => sum + d.financial.profitMargin, 0) / filteredData.length : 0
    },
    cashFlow: {
      inflow: totalRevenue,
      outflow: totalCosts,
    ...
    kpis: {
      ...
      avgRevenuePerLoad: filteredData.length > 0 ? totalRevenue / filteredData.length : 0,
      avgCostPerMile: filteredData.length > 0 ? 
        filteredData.reduce((sum, d) => sum + d.financial.costPerMile, 0) / filteredData.length : 0,
      avgRevenuePerMile: filteredData.length > 0 ? 
        filteredData.reduce((sum, d) => sum + d.financial.revenuePerMile, 0) / filteredData.length : 0,
...
export const getSettlementSummary = () => {
  const totalPending = SETTLEMENTS_DB.filter(s => s.status === 'pending').reduce((sum, s) => sum + s.netPay, 0);
  const totalApproved = SETTLEMENTS_DB.filter(s => s.status === 'approved').reduce((sum, s) => sum + s.netPay, 0);
  const totalPaid = SETTLEMENTS_DB.filter(s => s.status === 'paid').reduce((sum, s) => sum + s.netPay, 0);
