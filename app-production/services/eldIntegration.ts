...
export interface HoSViolation {
  id: string
  type: 'DRIVING_LIMIT' | 'ON_DUTY_LIMIT' | 'CYCLE_LIMIT' | 'REQUIRED_BREAK' | 'SLEEPER_BERTH'
...
class ELDIntegrationService {
  ...
  private async detectELDDevice(): Promise<void> {
    try {
      ...
      console.log('ELD device detected and connected')
    } catch (error) {
  ...
  async changeDutyStatus(
    ...
    try {
      ...
      localStorage.setItem('duty_status_history', JSON.stringify(this.dutyStatusHistory))
  ...
  private updateHoursOfService() {
    ...
    const remainingCycleTime = Math.max(0, 70 * 60 - this.calculateWeeklyOnDuty()) // 70 hour cycle
    ...
    // Save to localStorage
    localStorage.setItem('hours_of_service', JSON.stringify(this.hoursOfService))
  ...
  private calculateWeeklyOnDuty(): number {
    ...
    for (let i = 0; i < weeklyRecords.length; i++) {
      ...
      if (record.status === 'DRIVING' || record.status === 'ON_DUTY_NOT_DRIVING') {
  ...
  private calculateNextRequiredBreak(): string {
    ...
    const breakTime = new Date(Date.now() + (8 * 60 - this.hoursOfService.drivingTime) * 60000)
  ...
  private calculateNextRequiredRest(): string {
    ...
    const restTime = new Date(Date.now() + (14 * 60 - this.hoursOfService.onDutyTime) * 60000)
  ...
  private addViolation(violation: HoSViolation) {
    ...
    const existingViolation = this.violations.find(v => v.id === violation.id)
  ...
  async createDVIR(
    ...
    const dvir: DVIRRecord = {
      ...
      defects,
      signature,
      status: defects.some(d => d.severity === 'CRITICAL') ? 'UNSAFE' :
              defects.some(d => d.severity === 'MAJOR') ? 'CONDITIONAL' : 'SAFE'
    }
  ...
  private async syncWithDevice() {
    ...
    try {
      ...
      this.eldStatus.batteryLevel = Math.max(0, (this.eldStatus.batteryLevel || 85) - 0.1)
  ...
  private notifyHoursUpdate() {
    if (this.hoursOfService) {
      this.listeners.hoursUpdate.forEach(listener => listener(this.hoursOfService!))
    }
  }
  ...
  removeStatusChangeListener(listener: (status: ELDStatus) => void) {
    this.listeners.statusChange = this.listeners.statusChange.filter(l => l !== listener)
  }

  removeDutyChangeListener(listener: (duty: DutyStatus) => void) {
    this.listeners.dutyChange = this.listeners.dutyChange.filter(l => l !== listener)
  }

  removeHoursUpdateListener(listener: (hours: HoursOfService) => void) {
    this.listeners.hoursUpdate = this.listeners.hoursUpdate.filter(l => l !== listener)
  }

  removeViolationListener(listener: (violation: HoSViolation) => void) {
    this.listeners.violation = this.listeners.violation.filter(l => l !== listener)
  }
  ...
  canStartDriving(): boolean {
    return this.hoursOfService ? this.hoursOfService.remainingDriveTime > 0 : false
  }
