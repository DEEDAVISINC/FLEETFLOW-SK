...
import { LoadWorkflow, workflowManager, WorkflowStepId } from '../../lib/workflowManager';
...
export default function DriverPortal() {
  ...
  const submitBOLToWorkflow = async (loadId: string, workflowData: any) => {
    try {
      ...
      const loadIdentifier = LoadIdentificationService.generateLoadIdentifiers({
        origin: load.origin,
        destination: load.destination,
        pickupDate: load.pickupDate,
        equipment: load.equipment,
        loadType: load.loadType as 'FTL' | 'LTL' | 'Partial' | 'Expedited' | 'Hazmat',
        brokerInitials: load.brokerName?.split(' ').map(n => n[0]).join('') || 'FL',
      ...
      const bolData = {
        ...
        deliveryDate: workflowData.deliveryTimestamp ? 
          ...
          new Date().toISOString().split('T')[0],
        deliveryTime: workflowData.deliveryTimestamp || new Date().toTimeString().split(' ')[0],
      ...
      alert(`❌ BOL submission failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return false;
    }
  };
    try {
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead style={{ background: 'linear-gradient(135deg, #f9fafb, #f3f4f6)' }}>
                    <tr>
                      <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: '12px', fontWeight: 'bold', color: '#374151' }}>📞 Board #</th>
                    ...
                    {availableLoads.map((load, index) => (
                      ...
                      >
                        ...
                        onMouseOut={(e) => {
                          ...
                          <td style={{ padding: '12px 16px', fontSize: '14px', fontWeight: 'bold', color: '#059669', fontFamily: 'monospace' }}>
                            {load.loadBoardNumber || '000000'}
                          </td>
                          <td style={{ padding: '12px 16px', fontSize: '14px', fontWeight: 'bold', color: '#111827' }}>
                            {load.id}
                          </td>
