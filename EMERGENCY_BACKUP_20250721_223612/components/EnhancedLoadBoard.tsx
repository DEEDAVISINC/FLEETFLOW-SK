...
import { getBrokerLoads, getDispatcherLoads, getLoadStats, getMainDashboardLoads, Load } from '../services/loadService';
...
export default function EnhancedLoadBoard() {
  ...
  const generateProperLoadId = (load: Load) => {
    try {
      ...
      const brokerInitials = currentUser.initials || currentUser.name?.split(' ').map(n => n[0]).join('') || 'FL';
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(load =>
        ...
        (load.shipperName && load.shipperName.toLowerCase().includes(searchTerm.toLowerCase()))
      )
    }
      ...
      <div style={{
        ...
        {filteredLoads.length === 0 ? (
          ...
          <>
            ...
            <div style={{
              display: 'grid',
              gridTemplateColumns: '80px 70px 1.2fr 1fr 100px 80px 90px 80px 90px',
              gap: '8px',
              padding: '10px 12px',
              background: 'rgba(255, 255, 255, 0.1)',
              borderRadius: '8px',
            ...
            {filteredLoads.map((load, index) => (
              ...
              >
                ...
                <div style={{ fontWeight: '600', color: '#60a5fa' }}>{load.properLoadId || load.id}</div>
                <div>
