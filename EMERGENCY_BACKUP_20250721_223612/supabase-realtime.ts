...
class RealtimeSubscriptionManager {
  ...
  subscribeToNotifications(
    userId: string,
    callback: (payload: RealtimePostgresChangesPayload<NotificationUpdate>) => void
  ): string {
  ...
  subscribeToDriverLocations(
    companyId: string,
    callback: (payload: RealtimePostgresChangesPayload<DriverLocationUpdate>) => void
  ): string {
    ...
    const channel = this.client
      ...
      .on('postgres_changes', {
        ...
        if (oldRecord.current_location !== newRecord.current_location) {
          ...
          callback(payload as RealtimePostgresChangesPayload<DriverLocationUpdate>)
        }
      })
      .subscribe()
  ...
  getConnectionStatus(): string {
    return this.client.realtime.connection?.readyState === 1 ? 'connected' : 'disconnected'
  }
...
export const getRealtimeManager = (options?: SubscriptionOptions): RealtimeSubscriptionManager => {
  if (!realtimeManager) {
...
export const useNotifications = (
  userId: string,
  onNotification: (payload: RealtimePostgresChangesPayload<NotificationUpdate>) => void,
  enabled = true
) => {
  const manager = getRealtimeManager()
  ...
  React.useEffect(() => {
    ...
    const subscriptionId = manager.subscribeToNotifications(userId, onNotification)
...
export const formatRealtimePayload = (payload: RealtimePostgresChangesPayload<any>) => {
  return {
...
export const isLoadStatusChange = (payload: RealtimePostgresChangesPayload<LoadUpdate>): boolean => {
...
export const isNewNotification = (payload: RealtimePostgresChangesPayload<NotificationUpdate>): boolean => {
  return payload.eventType === 'INSERT'
}

export const isLocationUpdate = (payload: RealtimePostgresChangesPayload<DriverLocationUpdate>): boolean => {
  return payload.eventType === 'UPDATE' &&
         payload.old &&
         payload.new &&
