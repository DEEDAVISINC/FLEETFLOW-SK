...
const ShipperInvoicesTable = ({ invoices }: { invoices: ShipperInvoice[] }) => (
  <div className="bg-white rounded-lg shadow overflow-hidden">
    <div className="px-6 py-4 border-b border-gray-200">
      <h3 className="text-lg font-semibold text-gray-900">Shipper Invoices</h3>
      <p className="text-sm text-gray-600">Invoices sent to shippers for freight services</p>
    </div>
          ...
          {invoices.map((invoice) => (
            <tr key={invoice.id} className="hover:bg-gray-50">
              <td className="px-6 py-4 whitespace-nowrap">
                ...
                <div className="text-sm text-gray-500">{invoice.shipperCompany}</div>
                <div className="text-xs text-gray-400">
                  {new Date(invoice.invoiceDate).toLocaleDateString()} - {new Date(invoice.dueDate).toLocaleDateString()}
                </div>
              </td>
              <td className="px-6 py-4">
                <div className="text-sm text-gray-900">
                  ...
                  <div className="text-xs text-gray-400">
                    {invoice.loadDetails.equipment} • {invoice.loadDetails.weight?.toLocaleString()} lbs
                    {invoice.loadDetails.miles && ` • ${invoice.loadDetails.miles} mi`}
                  </div>
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                  invoice.agingCategory === 'Current' ? 'bg-green-100 text-green-800' :
                  invoice.agingCategory === '1-30 Days' ? 'bg-yellow-100 text-yellow-800' :
                  invoice.agingCategory === '31-60 Days' ? 'bg-orange-100 text-orange-800' :
