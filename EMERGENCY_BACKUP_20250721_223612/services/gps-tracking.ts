...
class GPSTrackingServiceClass {
  ...
  async startTracking(driverId: string): Promise<boolean> {
    try {
      ...
      if (this.isTracking[driverId]) {
        console.log(`Already tracking driver ${driverId}`);
        return true;
      }
      ...
      console.log(`Started GPS tracking for driver ${driverId}`);
      return true;
    } catch (error) {
  ...
  async stopTracking(driverId: string): Promise<boolean> {
    try {
      ...
      console.log(`Stopped GPS tracking for driver ${driverId}`);
      return true;
    } catch (error) {
  ...
  async updateLocation(driverId: string, location: Partial<GPSLocation>): Promise<boolean> {
    try {
  ...
  async getLocationHistory(driverId: string, hours: number = 24): Promise<GPSLocation[]> {
    try {
      ...
      return history.locations.filter(loc => new Date(loc.timestamp) >= cutoffTime);
    } catch (error) {
  ...
  async getETA(driverId: string, destinationLat: number, destinationLon: number): Promise<{ eta: string; distance: number }> {
    try {
      const currentLocation = this.locations[driverId];
      if (!currentLocation) {
      ...
      const distance = await this.calculateDistance(currentLocation, destination);
  ...
  private async processPosition(position: GeolocationPosition, driverId: string): Promise<GPSLocation> {
  ...
  private async reverseGeocode(lat: number, lon: number): Promise<{ city?: string; state?: string; address?: string }> {
    try {
  ...
  private handleLocationError(driverId: string, error: GeolocationPositionError): void {
