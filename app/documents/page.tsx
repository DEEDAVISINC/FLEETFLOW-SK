...
export default function DocumentsPage() {
  const [activeView, setActiveView] = useState<'overview' | 'rate-confirmation' | 'bill-of-lading' | 'templates' | 'batch-processing' | 'compliance-check'>('overview');
  ...
  const documentTemplates = {
    'hazmat-rate-confirmation': {
      ...
      description: 'Template for hazardous materials shipments with additional safety requirements',
      fields: ['hazmat_class', 'emergency_contact', 'placarding_requirements']
    },
    'oversized-load': {
      name: 'Oversized Load Documentation',
      description: 'Template for oversized/overweight loads requiring special permits',
    ...
    'refrigerated-transport': {
      ...
      fields: ['temperature_range', 'monitoring_requirements', 'backup_systems']
    },
    'international-shipment': {
      name: 'International Shipment',
      description: 'Template for cross-border shipments with customs requirements',
      fields: ['customs_forms', 'export_documentation', 'duty_information']
    }
  };

  // Validation Functions
  const validateDocument = (docType: 'rate-confirmation' | 'bill-of-lading') => {
    const errors: string[] = [];
    ...
    if (docType === 'bill-of-lading') {
      ...
      if (!selectedLoad?.destination) errors.push('Destination address is required');
  ...
  const autoSaveDocument = (docType: string, data: any) => {
    ...
    const savedDocs = JSON.parse(localStorage.getItem('fleetflow-documents') || '[]');
    savedDocs.push(savedDoc);
    localStorage.setItem('fleetflow-documents', JSON.stringify(savedDocs));
  };

  // Auto-generate document function
  const autoGenerateDocument = (docType: 'rate-confirmation' | 'bill-of-lading' | 'invoice' | 'proof-of-delivery') => {
    if (!selectedLoad) {
    ...
    if (!validateDocument(docType === 'invoice' ? 'rate-confirmation' : docType === 'proof-of-delivery' ? 'bill-of-lading' : docType)) {
      return;
    }
    ...
    alert(`${docType.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())} generated successfully for Load #${selectedLoad.id}!`);
  ...
  const quickGenerateDocument = (docType: 'rate-confirmation' | 'bill-of-lading' | 'invoice' | 'proof-of-delivery') => {
    if (!selectedLoad) {
      alert('Please select a load first');
      return;
    }
    ...
    alert(`${docType.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())} auto-generated for Load #${selectedLoad.id}!`);
  };
  ...
  const detectSpecialtyLoad = (load: any) => {
    ...
    // Check for hazmat
    if (load.hazmat || load.commodityClass?.includes('HAZ') || load.description?.toLowerCase().includes('hazmat')) {
      specialties.push('hazmat');
    }
    
    // Check for oversized
    if (load.oversized || load.weight > 80000 || load.dimensions?.length > 53 || load.description?.toLowerCase().includes('oversized')) {
      specialties.push('oversized');
    }
    
    // Check for refrigerated
    if (load.refrigerated || load.temperature || load.equipment?.toLowerCase().includes('reefer') || load.description?.toLowerCase().includes('refrigerated')) {
      specialties.push('refrigerated');
    }
    
    // Check for international
    if (load.international || load.origin?.includes('Canada') || load.destination?.includes('Canada') || load.origin?.includes('Mexico') || load.destination?.includes('Mexico')) {
      specialties.push('international');
    }
    
    // Check for high-value
    if (load.declaredValue > 100000 || load.description?.toLowerCase().includes('high value') || load.description?.toLowerCase().includes('electronics')) {
      specialties.push('high-value');
    }
    
    // Check for liquid bulk
    if (load.equipment?.toLowerCase().includes('tanker') || load.description?.toLowerCase().includes('liquid') || load.description?.toLowerCase().includes('bulk')) {
  ...
  const generateSpecialtyDocument = (specialtyType: string) => {
    ...
    const specialtyRequirements = {
      hazmat: {
        fields: ['hazmat_class', 'emergency_contact', 'placarding_requirements', 'msds_sheet'],
        description: 'Hazmat documentation with safety requirements'
      },
      oversized: {
        fields: ['permit_numbers', 'escort_requirements', 'route_restrictions', 'bridge_analysis'],
        description: 'Oversized load permits and routing documentation'
      },
      refrigerated: {
        fields: ['temperature_range', 'monitoring_requirements', 'backup_systems', 'pre_cool_time'],
        description: 'Temperature-controlled shipment documentation'
      },
      international: {
        fields: ['customs_forms', 'export_documentation', 'duty_information', 'broker_info'],
        description: 'Cross-border shipment documentation'
      },
      'high-value': {
        fields: ['security_requirements', 'insurance_coverage', 'tracking_systems', 'sealed_trailer'],
        description: 'High-value cargo security documentation'
      },
      'liquid-bulk': {
        fields: ['tank_specifications', 'loading_procedures', 'unloading_procedures', 'safety_protocols'],
        description: 'Liquid bulk transport documentation'
      }
    };

    const specialty = specialtyRequirements[specialtyType as keyof typeof specialtyRequirements];
    if (!specialty) return;
    ...
    alert(`${specialtyType.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())} specialty documentation generated for Load #${selectedLoad.id}!`);
  };

  // Calculate stats from loadHistory
  const stats = {
    totalLoads: loadHistory.length,
    documentsGenerated: loadHistory.filter(load => load.status === 'Delivered').length,
    inProgress: loadHistory.filter(load => load.status === 'In Transit').length,
    pendingDocuments: loadHistory.filter(load => load.status === 'Assigned').length,
    totalValue: loadHistory.reduce((sum, load) => sum + (typeof load.rate === 'number' ? load.rate : 0), 0),
    complianceScore: 98.5, // Simulated compliance score
    averageProcessingTime: 12, // minutes
    documentAccuracy: 99.2, // percentage
    specialtyLoads: loadHistory.filter(load => detectSpecialtyLoad(load).length > 0).length
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {[
              { id: 'overview', label: '📊 Overview', icon: '📊', color: '#3b82f6', bgColor: '#1e40af' },
              { id: 'rate-confirmation', label: '📄 Rate Confirmation', icon: '📄', color: '#10b981', bgColor: '#059669' },
              { id: 'bill-of-lading', label: '📋 Bill of Lading', icon: '📋', color: '#f59e0b', bgColor: '#d97706' },
              { id: 'templates', label: '🎨 Templates', icon: '🎨', color: '#8b5cf6', bgColor: '#7c3aed' },
              { id: 'batch-processing', label: '⚡ Batch Processing', icon: '⚡', color: '#06b6d4', bgColor: '#0891b2' },
              { id: 'compliance-check', label: '✅ Compliance Check', icon: '✅', color: '#ef4444', bgColor: '#dc2626' }
            ].map((tab) => (
              <button
                key={tab.id}
                ...
                style={{
                  ...
                  boxShadow: activeView === tab.id ? `0 8px 25px ${tab.color}60` : `0 2px 8px ${tab.color}30`
                }}
                onMouseOver={(e) => {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
                {loadHistory && loadHistory.length > 0 ? loadHistory.map((load) => (
                  <div
                    key={load.id}
                    onClick={(e) => {
                      e.preventDefault();
                    ...
                    style={{
                      background: selectedLoad?.id === load.id ? 'rgba(34, 197, 94, 0.2)' : 'rgba(255, 255, 255, 0.08)',
                      border: selectedLoad?.id === load.id ? '2px solid rgba(34, 197, 94, 0.5)' : '1px solid rgba(255, 255, 255, 0.15)',
                      borderRadius: '12px',
                      padding: '20px',
                      cursor: 'pointer',
                      transition: 'all 0.3s ease',
                    ...
                    <div style={{ display: 'grid', gap: '12px' }}>
                          ...
                          {detectSpecialtyLoad(load).map((specialty) => (
                            <span
                              ...
                              style={{
                                ...
                                borderRadius: '8px',
                                fontWeight: '600',
                                background: specialty === 'hazmat' ? '#ef444420' : 
                                           specialty === 'oversized' ? '#f59e0b20' :
                                           specialty === 'refrigerated' ? '#06b6d420' :
                                           specialty === 'international' ? '#8b5cf620' :
                                           specialty === 'high-value' ? '#10b98120' :
                                           specialty === 'liquid-bulk' ? '#3b82f620' : '#6b728020',
                                color: specialty === 'hazmat' ? '#ef4444' : 
                                       specialty === 'oversized' ? '#f59e0b' :
                                       specialty === 'refrigerated' ? '#06b6d4' :
                                       specialty === 'international' ? '#8b5cf6' :
                                       specialty === 'high-value' ? '#10b981' :
                                       specialty === 'liquid-bulk' ? '#3b82f6' : '#6b7280',
                                border: `1px solid ${specialty === 'hazmat' ? '#ef4444' : 
                                                   specialty === 'oversized' ? '#f59e0b' :
                                                   specialty === 'refrigerated' ? '#06b6d4' :
                                                   specialty === 'international' ? '#8b5cf6' :
                                                   specialty === 'high-value' ? '#10b981' :
                                                   specialty === 'liquid-bulk' ? '#3b82f6' : '#6b7280'}40`
                              }}
                            >
                              {specialty === 'hazmat' ? '⚠️ HAZMAT' : 
                               specialty === 'oversized' ? '📏 OVERSIZED' :
                               specialty === 'refrigerated' ? '🧊 REEFER' :
                               specialty === 'international' ? '🌍 INTL' :
                               specialty === 'high-value' ? '💎 HIGH-VALUE' :
                               specialty === 'liquid-bulk' ? '🛢️ BULK' : specialty.toUpperCase()}
                            </span>
                          ))}
                        </div>
                      )}
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '12px', borderTop: '1px solid rgba(255, 255, 255, 0.1)' }}>
                        ...
                        <span style={{ fontSize: '14px', fontWeight: 'bold', color: '#4ade80' }}>
                          {typeof load.rate === 'string' ? load.rate : formatCurrency(load.rate)}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div style={{ marginTop: '24px' }}>
                {selectedLoad ? (
                  <div style={{
                    ...
                    <p style={{ color: 'rgba(255, 255, 255, 0.9)', fontSize: '14px', margin: '8px 0 16px 32px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '12px', marginTop: '16px' }}>
                      <button
                        onClick={() => autoGenerateDocument('rate-confirmation')}
                        style={{
                    ...
                    {detectSpecialtyLoad(selectedLoad).length > 0 && (
                      <div style={{ marginTop: '16px' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                          {detectSpecialtyLoad(selectedLoad).map((specialty) => (
                            <button
                              key={specialty}
                              onClick={() => generateSpecialtyDocument(specialty)}
                              style={{
                                background: `linear-gradient(135deg, ${specialty === 'hazmat' ? '#ef4444, #dc2626' : 
                                                                     specialty === 'oversized' ? '#f59e0b, #d97706' :
                                                                     specialty === 'refrigerated' ? '#06b6d4, #0891b2' :
                                                                     specialty === 'international' ? '#8b5cf6, #7c3aed' :
                                                                     specialty === 'high-value' ? '#10b981, #059669' :
                                                                     specialty === 'liquid-bulk' ? '#3b82f6, #1e40af' : '#6b7280, #4b5563'})`,
                                color: 'white',
                                padding: '8px 16px',
                                borderRadius: '6px',
                                border: 'none',
                            ...
                            >
                              {specialty === 'hazmat' ? '⚠️ Generate Hazmat Docs' : 
                               specialty === 'oversized' ? '📏 Generate Oversized Permits' :
                               specialty === 'refrigerated' ? '🧊 Generate Reefer Docs' :
                               specialty === 'international' ? '🌍 Generate International Docs' :
                               specialty === 'high-value' ? '💎 Generate Security Docs' :
                               specialty === 'liquid-bulk' ? '🛢️ Generate Bulk Docs' : `Generate ${specialty} Docs`}
                            </button>
                          ))}
                        </div>
                      </div>
            ...
            <div style={{
              ...
              <h3 style={{ fontSize: '24px', fontWeight: '600', color: 'white', margin: '0 0 24px 0' }}>
                Quick Actions
              </h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                ...
                >
                  ...
                  <div style={{ fontSize: '16px', fontWeight: '600' }}>Generate Invoice</div>
                </button>
                ...
                >
                  <div style={{ fontSize: '32px', marginBottom: '12px' }}>📦</div>
                  <div style={{ fontSize: '16px', fontWeight: '600' }}>Generate Proof of Delivery</div>
                </button>
                ...
                <button
                  onClick={() => {
                    ...
                    if (savedDocs.length === 0) {
                      alert('No documents found. Generate some documents first!');
                    } else {
                      const recentDocs = savedDocs.slice(-5).reverse();
                      const docsList = recentDocs.map((doc: any) => `${doc.type} - Load #${doc.loadId} - ${new Date(doc.timestamp).toLocaleString()}`).join('\n');
                ...
                >
                  <div style={{ fontSize: '32px', marginBottom: '12px' }}>📚</div>
                  <div style={{ fontSize: '16px', fontWeight: '600' }}>View Recent Documents</div>
                </button>
                ...
                >
                  <div style={{ fontSize: '32px', marginBottom: '12px' }}>✅</div>
                  <div style={{ fontSize: '16px', fontWeight: '600' }}>Validate Documents</div>
                </button>
              </div>
            </div>
          </div>
        ...
        {activeView === 'templates' && (
          <div style={{
            ...
            <h2 style={{ fontSize: '28px', fontWeight: '600', color: 'white', margin: '0 0 24px 0' }}>
              🎨 Document Templates
            </h2>
            <p style={{ color: 'rgba(255, 255, 255, 0.8)', marginBottom: '32px' }}>
              Choose from pre-built templates optimized for specific freight types and requirements.
            </p>
        ...
        {activeView === 'batch-processing' && (
          <div style={{
            ...
            <p style={{ color: 'rgba(255, 255, 255, 0.8)', marginBottom: '32px' }}>
              Generate multiple documents simultaneously for efficient processing.
            </p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px', marginBottom: '32px' }}>
              {loadHistory.map((load) => (
                <div
                  key={load.id}
                  style={{
                    background: batchLoads.includes(load.id) ? 'rgba(34, 197, 94, 0.15)' : 'rgba(255, 255, 255, 0.03)',
                    border: batchLoads.includes(load.id) ? '1px solid rgba(34, 197, 94, 0.3)' : '1px solid rgba(255, 255, 255, 0.1)',
                    borderRadius: '12px',
                    padding: '16px',
                    cursor: 'pointer',
                    transition: 'all 0.3s ease'
            ...
            <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
              <button
                ...
                style={{
                  background: batchLoads.length === 0 ? 'rgba(255, 255, 255, 0.1)' : 'linear-gradient(135deg, #f59e0b, #d97706)',
                  color: batchLoads.length === 0 ? 'rgba(255, 255, 255, 0.5)' : 'white',
                  padding: '16px 32px',
                  borderRadius: '12px',
                  border: 'none',
                  fontSize: '16px',
        ...
        {activeView === 'compliance-check' && (
          <div style={{
            ...
            <p style={{ color: 'rgba(255, 255, 255, 0.8)', marginBottom: '32px' }}>
              Verify document compliance with DOT, FMCSA, and industry regulations.
            </p>
        ...
        {activeView === 'rate-confirmation' && (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px' }}>
              <h2 style={{ fontSize: '28px', fontWeight: '600', color: 'white', margin: 0 }}>
            ...
            {validationErrors.length > 0 && (
              <div style={{
                ...
                <h4 style={{ color: '#ef4444', fontSize: '16px', fontWeight: '600', margin: '0 0 8px 0' }}>
                  Validation Errors:
                </h4>
                <ul style={{ color: 'rgba(255, 255, 255, 0.8)', margin: 0, paddingLeft: '20px' }}>
