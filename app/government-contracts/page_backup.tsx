...
import USAspendingService, { SpendingRecord } from '../services/USAspendingService';
...
export default function GovernmentContractsPage() {
      ...
      const mockContracts: ContractOpportunity[] = [
        {
          ...
          description: 'Comprehensive transportation and logistics services for federal distribution centers across the Southwest region.',
          amount: 2500000,
        ...
        },
        {
          ...
          description: 'Specialized heavy-haul transportation for military equipment and vehicles between bases.',
          amount: 4200000,
        ...
        },
        {
          ...
          description: 'Regular mail and package transportation services for postal distribution network.',
          amount: 1800000,
        ...
        },
        {
          ...
          description: 'Emergency transportation and logistics support for disaster response operations.',
          amount: 3100000,
        ...
        },
        {
          ...
          description: 'Temperature-controlled transportation of medical supplies and pharmaceuticals.',
          amount: 1950000,
  ...
  const filteredAndSortedContracts = React.useMemo(() => {
    let filtered = contracts.filter(contract => {
      ...
      const matchesCompetition = !competitionFilter || contract.competitionLevel === competitionFilter;
    ...
    return filtered;
  }, [contracts, searchTerm, statusFilter, competitionFilter, sortBy, sortDirection]);
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '1.5rem', marginBottom: '10px' }}>Loading Government Contracts...</div>
          <div style={{ fontSize: '1rem', opacity: 0.8 }}>Analyzing opportunities and competitive landscape</div>
        </div>
      </div>
    );
  }
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 20px 40px 20px' }}>
        ...
        <div style={{
          ...
          }}>🏛️ Government Contract Intelligence</h1>
          <p style={{
            margin: 0,
            opacity: 0.9,
            fontSize: '1.1rem'
          }}>
            Strategic government contract opportunities with AI-powered competitive analysis and bidding insights
          </p>
        </div>
        ...
        <div style={{
          ...
          {[
            { label: 'Active Opportunities', value: contracts.filter(c => c.status === 'Open').length.toString(), color: '#10b981', icon: '📋' },
            { label: 'Total Value', value: formatCurrency(contracts.reduce((sum, c) => sum + c.amount, 0)), color: '#3b82f6', icon: '💰' },
            { label: 'High Match Score', value: contracts.filter(c => c.matchScore >= 90).length.toString(), color: '#22c55e', icon: '🎯' },
            { label: 'Closing Soon', value: contracts.filter(c => c.status === 'Closing Soon').length.toString(), color: '#f59e0b', icon: '⏰' }
          ].map((stat, index) => (
            <div key={index} style={{
              background: 'rgba(255, 255, 255, 0.1)',
              borderRadius: '15px',
              ...
              <div style={{ fontSize: '1.8rem', fontWeight: '700', color: stat.color, marginBottom: '5px' }}>{stat.value}</div>
        ...
        <div style={{
          ...
          <div style={{
            display: 'grid',
            gridTemplateColumns: '40px 180px 200px 120px 150px 100px 100px 80px 120px',
            background: '#f5f5f5',
          ...
          <div style={{ maxHeight: '600px', overflowY: 'auto' }}>
            {filteredAndSortedContracts.map((contract, index) => (
              ...
              >
                ...
                <div style={{ padding: '12px 10px', borderRight: '1px solid #e0e0e0' }}>
                  <div style={{ fontWeight: '600', marginBottom: '4px' }}>{contract.agency}</div>
                  <div style={{ fontSize: '10px', color: '#666' }}>{contract.naicsCode}</div>
                </div>
                <div style={{ padding: '12px 10px', borderRight: '1px solid #e0e0e0' }}>
                ...
                <div style={{ padding: '12px 10px', borderRight: '1px solid #e0e0e0', fontWeight: '700', color: '#10b981' }}>
                  {formatCurrency(contract.amount)}
                </div>
                <div style={{ padding: '12px 10px', borderRight: '1px solid #e0e0e0', fontSize: '11px' }}>
                  {contract.location}
                </div>
                <div style={{ padding: '12px 10px', borderRight: '1px solid #e0e0e0', fontSize: '11px' }}>
                  <div style={{ color: contract.status === 'Closing Soon' ? '#f59e0b' : '#333' }}>
                    {contract.deadline}
                  </div>
                </div>
                <div style={{ padding: '12px 10px', borderRight: '1px solid #e0e0e0' }}>
                  <span style={{
                    ...
                    fontSize: '10px',
                    fontWeight: '600',
                    backgroundColor: contract.competitionLevel === 'Low' ? '#d4edda' : 
                                    contract.competitionLevel === 'Medium' ? '#fff3cd' : '#f8d7da',
                    color: contract.competitionLevel === 'Low' ? '#155724' : 
                           contract.competitionLevel === 'Medium' ? '#856404' : '#721c24'
                  }}>
                ...
                <div style={{ padding: '12px 10px', borderRight: '1px solid #e0e0e0', textAlign: 'center' }}>
                  <span style={{
                    ...
                    fontSize: '10px',
                    fontWeight: '600',
                    backgroundColor: contract.matchScore >= 90 ? '#d4edda' : 
                                    contract.matchScore >= 80 ? '#fff3cd' : '#f8d7da',
                    color: contract.matchScore >= 90 ? '#155724' : 
                           contract.matchScore >= 80 ? '#856404' : '#721c24'
                  }}>
                    {contract.matchScore}%
                  </span>
                </div>
        ...
        <div style={{
          ...
          <div>
            Displaying {filteredAndSortedContracts.length} of {contracts.length} government contracts
          </div>
          <div>
