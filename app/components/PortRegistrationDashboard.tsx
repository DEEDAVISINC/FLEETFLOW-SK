...
export default function PortRegistrationDashboard() {
  ...
  const completedSteps = registrationSteps.filter(
  ...
  useEffect(() => {
    ...
    const urgentIncomplete = registrationSteps.filter(
      (s) => s.priority === 'urgent' && s.status !== 'completed'
    ).length;
    const highIncomplete = registrationSteps.filter(
              ...
              >
                {step.status !== 'completed' && (
                  <button
                    ...
                    style={{
                      background:
                        step.status === 'in_progress' ? '#10b981' : '#3b82f6',
                      color: 'white',
                      border: 'none',
                      padding: '8px 16px',
                      borderRadius: '6px',
