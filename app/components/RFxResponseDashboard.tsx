...
import {
  ...
  RFxResponse,
  rfxResponseService,
} from '../services/RFxResponseService';

interface RFxResponseDashboardProps {
  embedded?: boolean;
}

const RFxResponseDashboard: React.FC<RFxResponseDashboardProps> = ({
  embedded = false,
}) => {
  const [activeTab, setActiveTab] = useState<
    'requests' | 'search' | 'intelligence' | 'responses' | 'analytics'
  >('requests');
  const [rfxRequests, setRFxRequests] = useState<RFxRequest[]>([]);
  const [searchResults, setSearchResults] = useState<RFxRequest[]>([]);
  const [selectedRequest, setSelectedRequest] = useState<RFxRequest | null>(
    null
  );
  ...
  const searchOpportunities = async () => {
    ...
    try {
      const opportunities =
        await rfxResponseService.searchRFxOpportunities(searchParams);
  ...
  const handleRequestSelect = async (request: RFxRequest) => {
    ...
    setLoading(true);
    try {
      const intelligence = await rfxResponseService.getMarketIntelligence(
        request.origin,
        request.destination,
        request.equipment
      );
    ...
    <div>
      ...
      {!embedded && (
        ...
        >
          ...
          >
            Intelligent bid generation for RFB, RFQ, RFP, and RFI requests with
              ...
              {loading ? (
                ...
                <div className='rounded-xl bg-white p-8 text-center shadow-sm'>
                  <p className='text-gray-600'>
                    No RFx requests found matching your filters.
                  </p>
                </div>
              ) : (
                rfxRequests.map((request) => (
                  <div
                    ...
                    className={`cursor-pointer rounded-xl border-2 bg-white p-6 shadow-sm transition-all hover:shadow-md ${
                  ...
                  >
                    <div className='mb-4 flex items-start justify-between'>
                      <div className='flex items-center space-x-3'>
                        ...
                        <div>
                          ...
                          <p className='text-sm text-gray-600'>
                            {request.shipperName} • {request.origin} →{' '}
                            {request.destination}
                          </p>
                        </div>
                      </div>
                      <div className='flex flex-col items-end space-y-2'>
                        ...
                        <span
                          className={`rounded-full px-2 py-1 text-xs font-medium ${getPriorityColor(request.priority)}`}
                        >
                          {request.priority}
                        </span>
                      </div>
            ...
            {selectedRequest ? (
                    ...
                    <div>
                      ...
                      <div className='mt-1 flex items-center space-x-2'>
                        <span className='text-xl'>
                          {getRFxTypeIcon(selectedRequest.type)}
                        </span>
                        <span className='font-semibold'>
                          {selectedRequest.type}
                        </span>
                        <span
                          className={`rounded-full px-2 py-1 text-xs font-medium ${getPriorityColor(selectedRequest.priority)}`}
                        >
                      ...
                      <div className='mt-2 rounded-lg bg-gray-50 p-3'>
                        <div className='flex items-center justify-between'>
                          ...
                          <span className='text-sm text-gray-600'>
                            {selectedRequest.distance} mi
                          </span>
                        </div>
                        <div className='mt-1 text-sm text-gray-600'>
                          Equipment: {selectedRequest.equipment} • Weight:{' '}
                      ...
                      <div>
                        ...
                        <div className='mt-2 space-y-1'>
                          <div className='text-sm text-gray-700'>
                            • Trend: {marketIntelligence.trendDirection}
                          </div>
                          <div className='text-sm text-gray-700'>
                            • Capacity Tightness:{' '}
                            {marketIntelligence.capacityTightness}%
                          </div>
                          <div className='text-sm text-gray-700'>
                            • Rate Range:{' '}
                            {formatCurrency(marketIntelligence.rateRange.low)} -{' '}
                            {formatCurrency(marketIntelligence.rateRange.high)}
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className='rounded-xl bg-white p-6 shadow-sm'>
                    ...
                    <div className='space-y-4'>
                      ...
                      <div>
                        ...
                        <div className='mt-2 space-y-1'>
                          <div className='text-sm text-gray-700'>
                            • Positioning: {bidStrategy.competitivePositioning}
                          </div>
                          <div className='text-sm text-gray-700'>
                            • Risk Level: {bidStrategy.riskAssessment.level}
                          </div>
                      ...
                      <button
                        onClick={() =>
                          handleSubmitResponse({
                            ...
                            serviceDescription:
                              bidStrategy.differentiators?.join('\n') || '',
                          })
                        }
              ...
              <div className='rounded-xl bg-white p-8 text-center shadow-sm'>
                ...
                <h3 className='mb-2 text-lg font-semibold text-gray-900'>
                  Select an RFx Request
                </h3>
                <p className='text-gray-600'>
                  Choose a request from the list to view details and generate a
                  bid strategy.
                </p>
              </div>
            )}
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                {[
                  {
                    key: 'government',
                    label: '🏛️ Government Contracts (SAM.gov)',
                    desc: 'Federal, state & local government RFPs',
                  },
                  {
                    key: 'industry',
                    label: '🏢 Industry Portals',
                    desc: 'Corporate shipper procurement platforms',
                  },
                  {
                    key: 'loadboards',
                    label: '📋 Load Boards',
                    desc: 'DAT, Truckstop.com, and other load boards',
                  },
                  {
                    key: 'enterprise',
                    label: '🏭 Enterprise Shippers',
                    desc: 'Large retail and manufacturing RFx portals',
                  },
                ].map((platform) => (
                  <label
                    key={platform.key}
                    style={{
                      ...
                      gap: '8px',
                      padding: '8px 12px',
                      background: searchParams.platforms.includes(platform.key)
                        ? '#eff6ff'
                        : '#f9fafb',
                      borderRadius: '8px',
                      border: searchParams.platforms.includes(platform.key)
                        ? '1px solid #3b82f6'
                        : '1px solid #e5e7eb',
                      cursor: 'pointer',
                      transition: 'all 0.2s',
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          ...
                          setSearchParams({
                            ...searchParams,
                            platforms: searchParams.platforms.filter(
                              (p) => p !== platform.key
                            ),
                          });
                        }
              ...
              >
                {searchResults.map((opportunity) => (
                  <div
                    ...
                    style={{
                      background:
                        selectedRequest?.id === opportunity.id
                          ? '#eff6ff'
                          : '#ffffff',
                      border:
                        selectedRequest?.id === opportunity.id
                    ...
                    >
                      ...
                      >
                        <span style={{ fontSize: '24px' }}>
                          {getRFxTypeIcon(opportunity.type)}
                        </span>
                        <div>
                          <h4
                            style={{
                          ...
                          >
                            {opportunity.shipperName} • {opportunity.origin} →{' '}
      ...
      {activeTab === 'intelligence' && (
        <div className='rounded-xl bg-white p-6 shadow-sm'>
          ...
          <p className='text-gray-600'>
            Live market intelligence and analytics will be displayed here.
          </p>
        </div>
      )}
      ...
      {activeTab === 'responses' && (
        <div className='rounded-xl bg-white p-6 shadow-sm'>
          ...
          <p className='text-gray-600'>
            Your submitted responses and their status will be displayed here.
          </p>
        </div>
      )}
      ...
      {activeTab === 'analytics' && (
        <div className='rounded-xl bg-white p-6 shadow-sm'>
          ...
          <p className='text-gray-600'>
            Win rates, revenue metrics, and performance analytics will be
            displayed here.
          </p>
        </div>
      )}
