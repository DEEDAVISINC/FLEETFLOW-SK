  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(`✅ Transfer successful! ${selectedUser.firstName} has been notified.`);
      }
    } catch (error) {
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
              ...
              >
                ...
                {users.filter(u => u.userId !== currentUser?.userId).map((user) => (
                  <option key={user.userId} value={user.userId} style={{ background: '#1e293b', color: 'white' }}>
                    {user.firstName} {user.lastName} ({user.department}) {user.isOnline ? '🟢' : '🔴'}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              >
                ...
                <option value="urgent" style={{ background: '#1e293b', color: 'white' }}>🟡 Urgent</option>
                <option value="immediate" style={{ background: '#1e293b', color: 'white' }}>🔴 Immediate</option>
              </select>
            </div>
          </div>
