...
export default function RateConfirmation() {
  ...
  useEffect(() => {
    if (selectedLoad) {
      setLoadInfo((prev) => ({
        ...
        lineHaulRate:
          typeof selectedLoad.rate === 'string'
            ? parseFloat(selectedLoad.rate.replace(/[$,]/g, '')) || 0
            : selectedLoad.rate,
        totalRate:
          typeof selectedLoad.rate === 'string'
            ? parseFloat(selectedLoad.rate.replace(/[$,]/g, '')) || 0
  ...
  const calculateTotalRate = () => {
    const total =
      loadInfo.lineHaulRate +
      loadInfo.fuelSurcharge +
      loadInfo.accessorialCharges;
  ...
  const generateRateConfirmation = () => {
    ...
    // Generate load identifier
    let loadIdentifier = '';
    let shipperId = '';
    try {
      const EDIService = require('../services/EDIService').default;
      const loadData = {
        id: loadInfo.loadId,
        shipperInfo: {
          companyName: loadInfo.shipperName,
        },
        equipment: loadInfo.equipmentType,
        weight: loadInfo.commodityWeight,
      };
      const loadId = EDIService.generateLoadIdFromData(loadData);
      loadIdentifier = loadId.fullId;
      shipperId = loadId.shipperId;
    } catch (error) {
      loadIdentifier = loadInfo.loadId;
      shipperId = 'N/A';
    }
    ...
    pdf.text('RATE CONFIRMATION', pageWidth / 2, yPosition, {
      align: 'center',
    });
    yPosition += 15;
    ...
    yPosition += 10;
    pdf.text(`Load Identifier: ${loadIdentifier}`, 20, yPosition);
    pdf.text(`Shipper ID: ${shipperId}`, pageWidth - 80, yPosition);
    yPosition += 20;
    ...
    yPosition += 7;
    pdf.text(
      'Phone: (555) 123-4567 | Email: dispatch@fleetflowapp.com',
      20,
      yPosition
    );
    yPosition += 20;
    ...
    yPosition += 7;
    pdf.text(
      `MC: ${loadInfo.carrierMC} | DOT: ${loadInfo.carrierDOT}`,
      20,
      yPosition
    );
    yPosition += 7;
    ...
    yPosition += 7;
    pdf.text(
      `${loadInfo.carrierCity}, ${loadInfo.carrierState} ${loadInfo.carrierZip}`,
      20,
      yPosition
    );
    yPosition += 7;
    pdf.text(
      `Phone: ${loadInfo.carrierPhone} | Email: ${loadInfo.carrierEmail}`,
      20,
      yPosition
    );
    yPosition += 20;
    ...
    yPosition += 7;
    pdf.text(
      `License #: ${loadInfo.driverLicenseNumber} (${loadInfo.driverLicenseState})`,
      20,
      yPosition
    );
    yPosition += 20;
    ...
    yPosition += 7;
    pdf.text(
      `${loadInfo.shipperCity}, ${loadInfo.shipperState} ${loadInfo.shipperZip}`,
      20,
      yPosition
    );
    yPosition += 7;
    pdf.text(
      `Contact: ${loadInfo.shipperContact} | Phone: ${loadInfo.shipperPhone}`,
      20,
      yPosition
    );
    yPosition += 7;
    pdf.text(
      `Date: ${loadInfo.pickupDate} | Time: ${loadInfo.pickupTimeEarliest} - ${loadInfo.pickupTimeLatest}`,
      20,
      yPosition
    );
    yPosition += 15;
    ...
    yPosition += 7;
    pdf.text(
      `${loadInfo.consigneeCity}, ${loadInfo.consigneeState} ${loadInfo.consigneeZip}`,
      20,
      yPosition
    );
    yPosition += 7;
    pdf.text(
      `Contact: ${loadInfo.consigneeContact} | Phone: ${loadInfo.consigneePhone}`,
      20,
      yPosition
    );
    yPosition += 7;
    pdf.text(
      `Date: ${loadInfo.deliveryDate} | Time: ${loadInfo.deliveryTimeEarliest} - ${loadInfo.deliveryTimeLatest}`,
      20,
      yPosition
    );
    yPosition += 20;
    ...
    pdf.setFont('helvetica', 'normal');
    pdf.text(
      `Equipment: ${loadInfo.equipmentType} | Length: ${loadInfo.equipmentLength}' | Weight: ${loadInfo.equipmentWeight} lbs`,
      20,
      yPosition
    );
    yPosition += 7;
    ...
    yPosition += 7;
    pdf.text(
      `Weight: ${loadInfo.commodityWeight} lbs | Pieces: ${loadInfo.commodityPieces} | Class: ${loadInfo.commodityClass}`,
      20,
      yPosition
    );
    yPosition += 7;
    ...
    pdf.setFont('helvetica', 'normal');
    pdf.text(
      `Line Haul Rate: $${loadInfo.lineHaulRate.toFixed(2)}`,
      20,
      yPosition
    );
    yPosition += 7;
    if (loadInfo.fuelSurcharge > 0) {
      pdf.text(
        `Fuel Surcharge: $${loadInfo.fuelSurcharge.toFixed(2)}`,
        20,
        yPosition
      );
      yPosition += 7;
    ...
    if (loadInfo.accessorialCharges > 0) {
      pdf.text(
        `Accessorial Charges: $${loadInfo.accessorialCharges.toFixed(2)}`,
        20,
        yPosition
      );
      yPosition += 7;
    ...
    if (loadInfo.factoring) {
      pdf.text(
        `Factoring Company: ${loadInfo.factoringCompany}`,
        20,
        yPosition
      );
      yPosition += 7;
    ...
    if (loadInfo.quickPayDiscount > 0) {
      pdf.text(
        `Quick Pay Discount: ${loadInfo.quickPayDiscount}%`,
        20,
        yPosition
      );
      yPosition += 7;
    ...
    // Save the PDF
    pdf.save(
      `Rate_Confirmation_${loadInfo.loadId || 'Draft'}_${new Date().toISOString().split('T')[0]}.pdf`
    );
  };
  ...
  return (
    <div style={{ color: 'white' }}>
      ...
      <div style={sectionStyle}>
        <h3
          style={{ margin: '0 0 20px 0', color: '#4ade80', fontSize: '18px' }}
        >
          Load Information
        </h3>
        <div style={gridStyle}>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, loadId: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, referenceNumber: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, brokerOrderNumber: e.target.value })
              }
              style={inputStyle}
            />
          </div>
        ...
        <div style={gridStyle}>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, shipperName: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, shipperContact: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, shipperPhone: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, shipperEmail: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          ...
          >
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, shipperAddress: e.target.value })
                }
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, shipperCity: e.target.value })
                }
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, shipperState: e.target.value })
                }
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, shipperZip: e.target.value })
                }
                style={inputStyle}
              />
            </div>
        ...
        <div style={gridStyle}>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, consigneeName: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, consigneeContact: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, consigneePhone: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, consigneeEmail: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          ...
          >
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, consigneeAddress: e.target.value })
                }
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, consigneeCity: e.target.value })
                }
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, consigneeState: e.target.value })
                }
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, consigneeZip: e.target.value })
                }
                style={inputStyle}
              />
            </div>
        ...
        <div style={gridStyle}>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, carrierName: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, carrierMC: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, carrierDOT: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, carrierPhone: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          ...
          >
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, carrierAddress: e.target.value })
                }
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, carrierCity: e.target.value })
                }
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, carrierState: e.target.value })
                }
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, carrierZip: e.target.value })
                }
                style={inputStyle}
              />
            </div>
        ...
        <div style={gridStyle}>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, driverName: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, driverPhone: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, driverEmail: e.target.value })
              }
              style={inputStyle}
            />
          </div>
        ...
        <div style={gridStyle}>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, pickupDate: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, pickupTimeEarliest: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, pickupTimeLatest: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, deliveryDate: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          ...
          <div style={gridStyle}>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({
                    ...loadInfo,
                    deliveryTimeEarliest: e.target.value,
                  })
                }
                style={inputStyle}
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({
                    ...loadInfo,
                    deliveryTimeLatest: e.target.value,
                  })
                }
                style={inputStyle}
              />
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, equipmentType: e.target.value })
                }
                style={inputStyle}
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, equipmentLength: e.target.value })
                }
                style={inputStyle}
              >
        ...
        <div style={gridStyle}>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, commodityWeight: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, commodityPieces: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, commodityClass: e.target.value })
              }
              style={inputStyle}
            />
          </div>
          ...
          <div style={gridStyle}>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, commodityValue: e.target.value })
                }
                style={inputStyle}
              />
            </div>
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setLoadInfo({
                      ...loadInfo,
                      commodityHazmat: e.target.checked,
                    })
                  }
            ...
            {loadInfo.commodityHazmat && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setLoadInfo({ ...loadInfo, hazmatClass: e.target.value })
                  }
                  style={inputStyle}
                />
              </div>
        ...
        <div style={gridStyle}>
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, paymentTerms: e.target.value })
              }
              style={inputStyle}
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, paymentMethod: e.target.value })
              }
              style={inputStyle}
            >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({
                  ...loadInfo,
                  quickPayDiscount: parseFloat(e.target.value) || 0,
                })
              }
              style={inputStyle}
            />
          ...
          <div>
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setLoadInfo({ ...loadInfo, factoring: e.target.checked })
                }
                style={{ width: '16px', height: '16px' }}
              />
              Factoring Company
        ...
        {loadInfo.factoring && (
          <div style={{ marginTop: '16px' }}>
            ...
            <input
              ...
              onChange={(e) =>
                setLoadInfo({ ...loadInfo, factoringCompany: e.target.value })
              }
              style={inputStyle}
            />
          </div>
      ...
      <div style={sectionStyle}>
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) =>
              setLoadInfo({ ...loadInfo, specialInstructions: e.target.value })
            }
            style={{
              ...inputStyle,
              height: '100px',
