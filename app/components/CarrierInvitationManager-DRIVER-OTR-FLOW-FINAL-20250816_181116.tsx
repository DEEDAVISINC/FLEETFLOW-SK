...
import CarrierInvitationService, {
  CarrierInvitation,
  InvitationAnalytics,
} from '../services/CarrierInvitationService';
...
export default function CarrierInvitationManager({
  currentUser,
}: CarrierInvitationManagerProps) {
  const [activeTab, setActiveTab] = useState<
    'send' | 'track' | 'analytics' | 'templates'
  >('send');
    ...
    >
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '16px' }}>
          Invite carriers to access the Driver OTR Flow Portal - track responses and analyze conversion rates
        </p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '32px' }}>
        {[
          { id: 'send', label: 'Send Invitations', icon: '📧' },
          { id: 'track', label: 'Track Invitations', icon: '📊' },
      ...
      {activeTab === 'send' && (
        <div>
          ...
          >
            <h3
              style={{ color: 'white', fontSize: '20px', marginBottom: '24px' }}
            >
              📧 Send New Invitation
            </h3>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setInvitationForm({
                        ...invitationForm,
                        invitationType: e.target.value as 'email' | 'sms',
                      })
                    }
                    style={{
                      width: '100%',
            ...
            {[
              {
                status: 'sent',
                label: 'Sent',
                count: invitations.filter((i) => i.status === 'sent').length,
                color: '#6b7280',
              },
              {
                status: 'opened',
                label: 'Opened',
                count: invitations.filter((i) => i.status === 'opened').length,
                color: '#f59e0b',
              },
              {
                status: 'started',
                label: 'Started',
                count: invitations.filter((i) => i.status === 'started').length,
                color: '#3b82f6',
              },
              {
                status: 'completed',
                label: 'Completed',
                count: invitations.filter((i) => i.status === 'completed')
                  .length,
                color: '#10b981',
              },
            ].map((stat, index) => (
