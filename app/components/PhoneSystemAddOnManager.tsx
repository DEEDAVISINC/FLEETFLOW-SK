...
export default function PhoneSystemAddOnManager() {
  ...
  const loadPhoneData = () => {
    try {
      ...
      const subscription = SubscriptionManagementService.getUserSubscription(
        user.id
      );
      ...
      const addons = SubscriptionManagementService.getAlacarteModules()
        ...
        .map((addon) => ({
          ...
          isActive: subscription
            ? SubscriptionManagementService.hasFeatureAccess(
                user.id,
                `phone.${addon.id}`
              )
            : false,
  ...
  const handleAddPhoneSystem = async (addonId: string) => {
    try {
      ...
      console.info(`Adding phone system: ${addonId}`);
      ...
      alert(
        '✅ Phone system add-on activated! Your new features are now available.'
      );
    } catch (error) {
  ...
  const getCurrentPhonePlan = (): string => {
    ...
    const tier = SubscriptionManagementService.getSubscriptionTier(
      currentSubscription.subscriptionTierId
    );
    if (!tier) return 'Unknown plan';

    // Check what's included
    if (tier.features.some((f) => f.includes('Unlimited phone')))
      return 'Enterprise Phone (Unlimited)';
    if (tier.features.some((f) => f.includes('500 phone minutes')))
      return 'Professional Phone (500 min)';
    if (tier.features.some((f) => f.includes('100 phone minutes')))
      return 'Basic Phone (100 min)';
              ...
              >
                ...
                <div
                  style={{
                    ...
                    fontWeight: '600',
                    color: getUsageColor(
                      phoneUsage.minutesUsed,
                      phoneUsage.minutesLimit
                    ),
                  }}
                >
      ...
      >
        ...
        >
          <p style={{ marginBottom: '8px' }}>
            • <strong>Overage Charges:</strong> $0.02/minute for calls,
            $0.05/SMS when limits exceeded
          </p>
          <p style={{ marginBottom: '8px' }}>
            • <strong>CRM Integration:</strong> All calls automatically logged
            and synced with customer records
          </p>
          <p style={{ marginBottom: '8px' }}>
            • <strong>Multi-Tenant:</strong> Each company gets their own phone
            number and caller ID
          </p>
          <p style={{ margin: 0 }}>
            • <strong>Professional Features:</strong> Call recording,
            transcription, and real-time monitoring included
          </p>
        </div>
      </div>
    </div>
