...
export default function BrokerCarrierNetworkManager({
  ...
  const [enhancedCarriers, setEnhancedCarriers] = useState<
    EnhancedCarrierData[]
  >([]);

  // Broker-specific relationship data
  const [brokerRelationships, setBrokerRelationships] = useState<
    BrokerCarrierRelationship[]
  >([]);

  const [selectedCarrier, setSelectedCarrier] =
    useState<EnhancedCarrierData | null>(null);
  const [selectedRelationship, setSelectedRelationship] =
    useState<BrokerCarrierRelationship | null>(null);
    ...
    const mockBrokerRelationships: BrokerCarrierRelationship[] = [
      {
        ...
        creditRating: 'A',
        brokerNotes:
          'Excellent carrier, always on time. Preferred for high-value loads.',
      ...
      },
      {
        ...
        creditRating: 'B',
        brokerNotes:
          'Good for specialized equipment. Reliable for flatbed loads.',
  ...
  const carriersWithRelationships = enhancedCarriers
    .map((carrier) => {
      const relationship = brokerRelationships.find(
        (rel) => rel.carrierId === carrier.id
      );
      return { carrier, relationship };
    })
    .filter((item) => item.relationship); // Only show carriers this broker has relationships with

  const filteredCarriers = carriersWithRelationships.filter(
    ({ carrier, relationship }) => {
      ...
      if (
        ...
        relationship?.relationshipType !== filters.relationshipType
      )
        return false;
    ...
    >
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '16px' }}>
          Manage relationships with carriers onboarded through Enhanced Carrier
          Portal
        </p>
      </div>
        ...
        {[
          {
            ...
            label: 'My Network',
            icon: '🚛',
            count: carriersWithRelationships.filter(
            ...
            {filteredCarriers.map(({ carrier, relationship }) => (
              ...
              >
                        ...
                        style={{
                          background:
                            relationship.relationshipType === 'preferred'
                              ? '#10b981'
                              : relationship.relationshipType === 'standard'
                                ? '#3b82f6'
                                : '#f59e0b',
                          color: 'white',
                          padding: '4px 12px',
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  >
                    {carrier.capacity.trucks} trucks •{' '}
                    {carrier.capacity.available} available
                  </div>
                </div>
      ...
      {activeTab === 'invite' && (
        <div>
          <div style={{ marginBottom: '24px', textAlign: 'center' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '16px' }}>
              All carriers must go through Enhanced Carrier Portal for
