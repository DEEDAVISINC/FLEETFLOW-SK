...
const LOCATION_TYPES = {
  'Distribution Center': {
    defaultSafetyRequirements:
      'High-vis vest required, steel-toe boots, hard hat in designated areas',
    ...
    defaultDocumentationRequirements:
      'BOL, delivery receipt, photo confirmation',
  ...
  'Construction Site': {
    defaultSafetyRequirements:
      'Hard hat, safety vest, steel-toe boots mandatory',
    defaultAccessRequirements: 'Site check-in required, safety briefing',
    defaultTimingRestrictions:
      'Daylight hours only, no deliveries during concrete pours',
    defaultDocumentationRequirements:
      'Delivery ticket, site supervisor signature',
...
const RouteGenerator: React.FC<RouteBuilderProps> = ({
  ...
  const convertLoadToRouteData = (
    load: RouteBuilderProps['selectedLoad']
  ): Partial<RouteData> => {
    if (!load) return {};
  ...
  const prePopulatedData = selectedLoad
    ? convertLoadToRouteData(selectedLoad)
    : {};
  ...
  useEffect(() => {
    const locationDefaults =
      LOCATION_TYPES[routeData.locationType as keyof typeof LOCATION_TYPES];
    if (locationDefaults) {
      setRouteData((prev) => ({
        ...prev,
        safetyRequirements:
  ...
  const generatePreview = () => {
    try {
      ...
      setErrors({
        general: 'Error generating preview. Please check your inputs.',
      });
      setPreviewDocument('');
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setErrors({
        general: 'Error generating route document. Please check your inputs.',
      });
    } finally {
  ...
  const sendRouteSMS = async (
    ...
    recipients: { name: string; phone: string; type: 'driver' | 'carrier' }[]
  ) => {
    try {
      const response = await fetch('/api/notifications/send', {
        ...
        body: JSON.stringify({
          loadData: {
            ...
            destination:
              routeData.stops.length > 0 ? routeData.stops[0].name : 'TBD',
        ...
        <div>
          ...
          {selectedLoad ? (
            <div className='text-gray-600'>
              <p>
                Generating route document for Load:{' '}
            ...
            <p className='text-gray-600'>
              Create professional route documents and delivery instructions
            </p>
          )}
        </div>
        {onClose && (
          <button
            onClick={onClose}
            className='px-4 py-2 text-xl text-gray-600 hover:text-gray-800'
          >
            ✕
          </button>
        )}
            ...
            <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange('routeName', e.target.value)
                  }
                  className='w-full rounded-md border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-blue-500'
                />
              </div>
              ...
              <div className='grid grid-cols-1 gap-4 md:grid-cols-3'>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange('pickupContact', e.target.value)
                    }
                    className='w-full rounded-md border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-blue-500'
                  />
                </div>
              ...
              <div className='space-y-4'>
                ...
                <div className='grid grid-cols-1 gap-2 md:grid-cols-3'>
                  <button
                    onClick={async () => {
                      try {
                        const driverPhone =
                          routeData.pickupPhone ||
                          prompt('Enter driver phone number:');
                        if (driverPhone) {
                          const result = await sendRouteSMS(previewDocument, [
                            {
                              name: routeData.driverName || 'Driver',
                              phone: driverPhone,
                              type: 'driver',
                            },
                          ]);
                  ...
                  <button
                    onClick={async () => {
                      try {
                        const carrierPhone = prompt(
                          'Enter carrier phone number:'
                        );
                        if (carrierPhone) {
                          const result = await sendRouteSMS(previewDocument, [
                            {
                              name: routeData.companyName || 'Carrier',
                              phone: carrierPhone,
                              type: 'carrier',
                            },
                          ]);
                          alert(
                            result.success
                              ? '✅ Route sent to carrier!'
                              : '❌ Failed to send SMS'
                          );
                        }
                      } catch (error) {
                        console.error('Error sending route to carrier:', error);
                        alert('❌ Failed to send SMS');
                      }
                    }}
                    ...
                    onClick={async () => {
                      try {
                        ...
                        if (driverPhone && carrierPhone) {
                          ...
                          alert(
                            result.success
                              ? '✅ Route sent to both parties!'
                              : '❌ Failed to send SMS'
                          );
                        }
                      } catch (error) {
                        console.error(
                          'Error sending route to both parties:',
                          error
                        );
                        alert('❌ Failed to send SMS');
                      }
                ...
                <div className='rounded-lg border border-blue-200 bg-blue-50 p-3'>
                  <div className='text-sm text-blue-800'>
                    💡 <strong>Pro Tip:</strong> Use the Carrier Verification
                    tab to automatically populate verified carrier contact
                    information.
                  </div>
                </div>
              </div>
