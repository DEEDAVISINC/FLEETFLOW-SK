...
export default function DriverLogin() {
  ...
  useEffect(() => {
    console.info('Login page mounted, AuthService:', AuthService);
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          console.info('Location permission denied:', error);
        }
      );
    }
  }, []);
  ...
  const handleDemoLogin = async (demoDriverId: string) => {
    console.info('Demo login button clicked for:', demoDriverId);
    setDebugInfo(`Attempting login for ${demoDriverId}...`);
    setIsLoading(true);
    setError('');

    try {
      console.info('Attempting demo login with:', demoDriverId);
      ...
      >
        ...
        <form onSubmit={handleLogin} style={{ marginBottom: '32px' }}>
          ...
          <button
            type='submit'
            disabled={
              isLoading || !credentials.driverId || !credentials.password
            }
            style={{
              width: '100%',
              padding: '16px',
        ...
        >
          ...
          >
            Loading: {isLoading ? 'Yes' : 'No'} | DeviceID:{' '}
            {deviceId ? 'Set' : 'Not Set'} | Location:{' '}
          ...
          >
            ...
            <button
              onClick={() => {
                console.info('DRV-001 button clicked!');
            ...
            >
              ...
              <span style={{ fontSize: '12px', opacity: 0.8 }}>Dallas, TX</span>
            </button>

            <button
              onClick={() => {
                console.info('DRV-002 button clicked!');
