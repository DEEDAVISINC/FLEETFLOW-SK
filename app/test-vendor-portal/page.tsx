...
export default function TestVendorPortalPage() {
  ...
  useEffect(() => {
    ...
    fetch('/vendor-portal')
      .then(response => {
        if (response.ok) {
          setPortalLoads(true);
          console.log('✅ Vendor portal page loads successfully');
        } else {
          setPortalLoads(false);
          console.log('❌ Vendor portal page failed to load:', response.status);
        }
      })
      .catch(error => {
        setPortalLoads(false);
  ...
  const testPortalAccess = () => {
    ...
    console.log('🧪 Test session set up');
  ...
  return (
    <div style={{ padding: '40px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ background: '#f3f4f6', padding: '20px', borderRadius: '8px', marginBottom: '20px' }}>
        ...
        {sessionData ? (
          <pre style={{ background: '#e5e7eb', padding: '12px', borderRadius: '4px', overflow: 'auto' }}>
      ...
      <div style={{ background: '#fef3c7', padding: '20px', borderRadius: '8px' }}>
        ...
        {portalLoads === true && <p style={{ color: 'green' }}>✅ Vendor portal page loads successfully</p>}
        {portalLoads === false && <p style={{ color: 'red' }}>❌ Vendor portal page failed to load</p>}
        {portalLoads === null && <p>Testing portal load...</p>}
      </div>
      
      <div style={{ background: '#dbeafe', padding: '20px', borderRadius: '8px', marginTop: '20px' }}>
        <h3>Instructions:</h3>
        <ol>
