...
export async function GET() {
  ...
  const apiResults = apiTests.map((result, index) => {
    ...
    if (result.status === 'fulfilled' && result.value.ok) {
      ...
      return { name: apiNames[index], status: 'error', message: result.status === 'rejected' ? result.reason : 'API not responding' };
    }
  });
  ...
  if (healthCheck.services.apis.status === 'degraded' && healthCheck.status === 'healthy') {
    healthCheck.status = 'degraded';
  }
