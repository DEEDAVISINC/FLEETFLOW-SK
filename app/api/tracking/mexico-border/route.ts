...
const MAJOR_BORDER_CROSSINGS: MexicoBorderCrossing[] = [
  {
    ...
    specialRequirements: ['QR Code Mandatory', 'C-TPAT Preferred', 'FAST Lane Available']
  },
  {
    ...
    specialRequirements: ['Enhanced Security', 'Drug Screening', 'X-Ray Required']
  },
  {
    ...
    coordinates: [32.5550, -117.0310],
    cbpPort: 'Otay Mesa, CA',
    mexicanPort: 'Tijuana, Baja California',
    specialRequirements: ['Produce Inspection', 'Phytosanitary Cert', 'High Volume Delays']
  },
  {
    ...
    specialRequirements: ['Standard Processing', 'Energy Cargo Hub', 'IMMEX Friendly']
  },
  {
    ...
    specialRequirements: ['Produce Inspection', 'Seasonal Delays', 'Limited Hours']
  }
];
...
function generateSAAIManifests(count: number = 50): SAAIManifest[] {
  ...
  const brokers = ['Agencia Aduanal López', 'Customs Broker Hernández', 'SAT Solutions', 'Mexico Trade Partners'];
...
function generateACEManifests(count: number = 50): ACEManifest[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const manifest: ACEManifest = {
      ...
      estimatedArrival: new Date(Date.now() + Math.random() * 48 * 60 * 60 * 1000).toISOString(),
  ...
  try {
    ...
    switch (type) {
      case 'crossings':
        ...
        if (crossing) {
          crossings = crossings.filter(c => c.code === crossing.toUpperCase());
        }
        return NextResponse.json({
          ...
          data: {
            ...
            totalThroughput: crossings.reduce((sum, c) => sum + c.throughput, 0),
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            summary: {
              ...
              approved: saaiManifests.filter(m => m.status === 'approved').length,
              processing: saaiManifests.filter(m => m.status === 'processing').length,
              rejected: saaiManifests.filter(m => m.status === 'rejected').length,
              averageProcessingTime: saaiManifests.reduce((sum, m) => sum + m.processingTime, 0) / saaiManifests.length
            }
          }
        });
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            summary: {
              ...
              accepted: aceManifests.filter(m => m.status === 'accepted').length,
              pending: aceManifests.filter(m => m.status === 'submitted').length,
              rejected: aceManifests.filter(m => m.status === 'rejected').length,
              qrCodeRequired: aceManifests.filter(m => m.qrCodeRequired).length,
              inspectionFlagged: aceManifests.filter(m => m.inspectionFlag).length
            }
          }
        });
            ...
            activeAlerts: [
              {
                ...
                message: 'QR Code ACE Manifest lead sheets are now mandatory at Port of Laredo effective August 2024',
              ...
              },
              {
                ...
                message: 'Experiencing 4+ hour delays due to system maintenance and increased cargo volume',
              ...
              },
              {
                ...
                message: 'Updated IMMEX documentation requirements effective January 2025',
              ...
              },
              {
                ...
                message: 'Enhanced security screening implemented for high-risk cargo categories',
              ...
              },
              {
                ...
                message: 'Potential weather delays expected this weekend due to winter storm system',
