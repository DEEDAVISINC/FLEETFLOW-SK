...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (type) {
      case 'summary':
        const summary = await portAuthorityService.getMaritimeIntelligenceSummary();
      ...
      case 'vessels':
        const vessels = await portAuthorityService.getNOADVesselData(port || undefined);
        return NextResponse.json({
          success: true,
          data: {
            vessels,
      ...
      case 'ports':
        ...
        const portPromises = majorPorts.map(async (portCode) => {
          try {
            return await portAuthorityService.getEnhancedPortIntelligence(portCode);
          } catch (error) {
            console.error(`Error fetching port intelligence for ${portCode}:`, error);
            return null;
          }
        });
        
        const portIntelligence = (await Promise.all(portPromises)).filter(Boolean);
      ...
      case 'schedules':
        const schedules = await portAuthorityService.getVesselSchedulesNOAD(port || undefined);
        return NextResponse.json({
          success: true,
          data: {
            schedules,
