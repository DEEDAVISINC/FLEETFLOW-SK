...
export async function GET(request: NextRequest) {
  return ServiceErrorHandler.handleAsyncOperation(async () => {
    ...
    const fullStatus = {
      ...
      client_id: process.env.PLAID_CLIENT_ID ? `${process.env.PLAID_CLIENT_ID.substring(0, 8)}...` : 'Not configured',
      ...
      integration_health: {
        sdk_installed: true,
        environment_variables_configured: !!(process.env.PLAID_CLIENT_ID && process.env.PLAID_SECRET),
    ...
    console.log(`✅ Plaid compliance check completed. Status: ${complianceStatus.overall_compliant ? 'COMPLIANT' : 'PENDING'}`);
    
    return NextResponse.json(fullStatus, { status: statusCode });
    
  }, 'PlaidComplianceAPI', 'GET') || 
  NextResponse.json(
    { error: 'Failed to check compliance status', timestamp: new Date().toISOString() }, 
    { status: 500 }
  );
}
...
export async function POST(request: NextRequest) {
  ...
  }, 'PlaidComplianceAPI', 'POST') ||
  NextResponse.json(
    { error: 'Failed to run compliance validation', timestamp: new Date().toISOString() }, 
    { status: 500 }
  );
}
