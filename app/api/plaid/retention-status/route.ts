  ...
  return (
    ServiceErrorHandler.handleAsyncOperation(
      async () => {
        ...
        const connectedUsers = plaidService.getConnectedUsers(
          tenantId || undefined
        );
        ...
        const retentionAnalytics = connectedUsers.reduce(
          (acc, user) => {
            const daysUntilExpiry = Math.floor(
              (user.data_retention_expiry.getTime() - now.getTime()) /
                (24 * 60 * 60 * 1000)
            );
        ...
        const userSpecificData = userId
          ? connectedUsers.find((u) => u.user_id === userId)
          : null;
        ...
        const response = {
          ...
          retention_timeline: {
            ...
            next_expiry_in_days:
              retentionAnalytics.shortest_retention_days === Infinity
                ? null
                : retentionAnalytics.shortest_retention_days,
        ...
        if (userSpecificData) {
          response['user_specific_data'] = {
            ...
            days_until_expiry: Math.floor(
              (userSpecificData.data_retention_expiry.getTime() -
                now.getTime()) /
                (24 * 60 * 60 * 1000)
            ),
...
export async function POST(request: NextRequest) {
  return (
        ...
        switch (action) {
          ...
          case 'force_deletion':
            ...
            return NextResponse.json({
              ...
              compliance_note:
                'Forced deletion overrides SOX retention requirements',
            });
          ...
          case 'bulk_cleanup':
            ...
            const now = new Date();
            const expiredUsers = connectedUsers.filter(
              (user) => user.data_retention_expiry <= now
            );
    ...
    NextResponse.json(
      {
        error: 'Failed to process retention management action',
        timestamp: new Date().toISOString(),
      },
      { status: 500 }
    )
