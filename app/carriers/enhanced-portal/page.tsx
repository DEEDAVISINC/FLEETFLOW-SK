...
const CarrierCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      {carrier.status !== 'active' && (
        <div style={{ marginBottom: '12px' }}>
          ...
          >
            ...
            >
              {carrier.onboardingProgress || 0}/5 Steps
            </span>
          </div>
          <div
            style={{
          ...
          >
            <div
              style={{
                ...
                height: '100%',
                width: `${((carrier.onboardingProgress || 0) + 1) * 20}%`,
      ...
      >
        <span>🏦 {carrier.factoringCompany || 'Not Set'}</span>
        <span>👥 {carrier.fleetSize || 0} Drivers</span>
      </div>
    </div>
  );
};
...
export default function EnhancedCarrierPortal() {
  ...
  const getStatusCounts = () => {
    ...
    const totalCarriers = carriers.length || 1; // Prevent division by zero
    const activeCarriers =
      carriers.filter((c) => c.status === 'active').length || 1;
    ...
    const portalAdoption = Math.round(
      (carriers.filter((c) => c.portalAccess).length / activeCarriers) * 100
    );
        ...
        {activeView === 'dashboard' && (
          ...
          >
            ...
            >
              ...
              {/* Get real onboarding pipeline data from carriers in progress */}
              {(() => {
                // Filter carriers currently in onboarding process
                const pipelineCarriers = carriers
                  .filter(
                    (c) => c.status === 'pending' || c.status === 'under_review'
                  )
                  .map((carrier) => ({
                    ...
                    currentStep: carrier.onboardingProgress || 0,
                    steps: [
                      'FMCSA',
                      'Documents',
                      'Factoring',
                    ...
                    estimatedCompletion: 'TBD', // TODO: Calculate based on current progress
                  }));
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {Math.round(
                      (carriers.filter((c) => c.insuranceVerified).length /
                        (carriers.length || 1)) *
                        100
                    )}
                    %
                ...
                >
                  ...
                  >
                    {Math.round(
                      (carriers.filter((c) => c.documentsUploaded).length /
                        (carriers.length || 1)) *
                        100
                    )}
                    %
                ...
                >
                  ...
                  >
                    {Math.round(
                      (carriers.filter((c) => c.factoringApproved).length /
                        (carriers.length || 1)) *
                        100
                    )}
                    %
                  ...
                  >
                    {
                      carriers.filter(
                        (c) =>
                          c.status === 'under_review' || !c.documentsUploaded
                      ).length
                    }
                  </div>
                  <div
              ...
              >
                ...
                <div style={{ display: 'grid', gap: '8px' }}>
                  {carriers
                    .filter((c) => !c.documentsUploaded || !c.insuranceVerified)
                    .slice(0, 5) // Limit to 5 alerts
                    .map((carrier) => ({
                      carrier: carrier.companyName,
                      doc: !carrier.insuranceVerified
            ...
            >
              ...
              >
                {[
                  ...
                  // Use actual carrier onboarding progress data
                  const currentProgress =
                    selectedCarrier.onboardingProgress || 0;
                  const isCompleted =
                    index < currentProgress ||
                    selectedCarrier.status === 'active';
                  const isCurrent = !isCompleted && index === currentProgress;
                  return (
                    <div
                      key={step}
                      style={{
