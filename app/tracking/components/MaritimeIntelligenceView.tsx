...
import { NOADVesselData, PortIntelligence, VesselSchedule } from '../../services/NOADService';
...
export default function MaritimeIntelligenceView({ shipments = [] }: MaritimeIntelligenceViewProps) {
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'overview' | 'vessels' | 'ports' | 'schedules'>('overview');
  const [summary, setSummary] = useState<MaritimeIntelligenceSummary | null>(null);
  ...
  const loadMaritimeData = async () => {
    ...
    try {
      // Load summary data
      const summaryData = await portAuthorityService.getMaritimeIntelligenceSummary();
      setSummary(summaryData);

      // Load vessel data
      const vessels = await portAuthorityService.getNOADVesselData(selectedPort === 'all' ? undefined : selectedPort);
      ...
      const schedules = await portAuthorityService.getVesselSchedulesNOAD(selectedPort === 'all' ? undefined : selectedPort);
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '18px', fontWeight: '500' }}>Loading Maritime Intelligence...</div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        >
          ...
          {majorPorts.map(port => (
            <option key={port.code} value={port.code} style={{ color: 'black' }}>
              {port.name}
            </option>
          ))}
        </select>
      ...
      <div style={{
        ...
        {[
          ...
          <button
            ...
            style={{
              background: activeTab === tab.id ? 'rgba(20, 184, 166, 0.3)' : 'rgba(255, 255, 255, 0.1)',
              color: activeTab === tab.id ? '#14b8a6' : 'rgba(255, 255, 255, 0.7)',
              border: activeTab === tab.id ? '1px solid #14b8a6' : '1px solid rgba(255, 255, 255, 0.2)',
              borderRadius: '8px',
              padding: '12px 20px',
              cursor: 'pointer',
              fontSize: '14px',
        ...
        <div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  ...
                  <p style={{ color: 'white', fontSize: '32px', fontWeight: 'bold', margin: 0 }}>
          ...
          {summary.criticalAlerts.length > 0 && (
            <div style={{
              ...
              <h3 style={{ color: '#fca5a5', fontSize: '18px', fontWeight: 'bold', margin: '0 0 16px 0' }}>
                🚨 Critical Alerts
              </h3>
          ...
          {summary.topCongestionPorts.length > 0 && (
            <div style={{
              ...
              <h3 style={{ color: 'white', fontSize: '18px', fontWeight: 'bold', margin: '0 0 16px 0' }}>
                🚧 Congested Ports
              </h3>
            ...
            {vesselData.slice(0, 10).map((vessel, index) => (
              <div key={index} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    <h4 style={{ color: 'white', fontSize: '18px', fontWeight: 'bold', margin: '0 0 8px 0' }}>
                      {vessel.vesselName}
                    </h4>
                    ...
                    <div style={{ display: 'flex', gap: '20px', marginBottom: '12px' }}>
                      ...
                      <span style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '14px' }}>
                        {vessel.noticeType}: {formatDateTime(vessel.noticeType === 'Arrival' ? vessel.arrivalDateTime : vessel.departureDateTime)}
                      </span>
                    </div>
                    {vessel.cargoManifest.length > 0 && (
                      <div style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '14px' }}>
                        Cargo: {vessel.cargoManifest[0].commodity} ({vessel.cargoManifest[0].weight.toLocaleString()} tons)
                      </div>
                    )}
                  </div>
            ...
            {portData.map((port, index) => (
              <div key={index} style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                  <div>
                    <div style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '12px' }}>Current Vessels</div>
                    <div style={{ color: 'white', fontSize: '18px', fontWeight: 'bold' }}>{port.currentVessels}</div>
                  </div>
                  <div>
                    <div style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '12px' }}>Berth Utilization</div>
                    <div style={{ color: 'white', fontSize: '18px', fontWeight: 'bold' }}>{port.berthUtilization}%</div>
                  </div>
                  <div>
                    <div style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '12px' }}>Wait Time</div>
                    <div style={{ color: 'white', fontSize: '18px', fontWeight: 'bold' }}>{port.averageWaitTime}h</div>
                  </div>
                  <div>
                    <div style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '12px' }}>Efficiency</div>
                    <div style={{ color: 'white', fontSize: '18px', fontWeight: 'bold' }}>{port.efficiency}%</div>
                  </div>
                </div>
                ...
                <div style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '14px' }}>
                  ...
                  <div>Cargo Volume: {port.cargoVolume24h.toLocaleString()} tons</div>
                </div>
              </div>
            ))}
          </div>
              ...
              <div key={index} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '14px' }}>
                      Operations: {schedule.cargoOperations.loading ? 'Loading' : ''} {schedule.cargoOperations.unloading ? 'Unloading' : ''} 
