...
export default function DispatchCentral() {
    ...
    if (currentUser) {
      ...
      const filteredLoads = userLoads.filter((load) => {
        const isAssignedToDispatcher =
          load.dispatcherId === currentUser.id || load.status === 'Available';
  ...
  const loadWorkflowStatuses = async (loads: Load[]) => {
    ...
    for (const load of loads) {
      try {
        const workflows = await workflowBackendService.getActiveWorkflows(
          load.id
        );
        if (workflows.length > 0) {
          ...
          statuses[load.id] = {
            ...
            driverName: workflow.driverId
              ? mockDrivers.find((d) => d.id === workflow.driverId)?.name ||
                'Unknown'
              : 'Unassigned',
        ...
        try {
          const integrationResult =
            await LoadScheduleIntegrationService.integrateLoadAssignment({
              ...
              priority: assignedLoad.priority || 'Medium',
              assignedBy: user.name || 'Dispatcher',
              customerId: assignedLoad.customerId,
              brokerName: assignedLoad.brokerName,
            });
              ...
              >
                {[
                  ...
                  },
                  {
                    ...
                    value: stats.driverSelected,
                    color: '#f97316',
                    trend: '+7%',
                  },
                  {
                    title: 'Order Sent',
                    value: stats.orderSent,
                    color: '#06b6d4',
                    trend: '+10%',
                  },
                  {
                ...
                >
                  ...
                  >
                    <input
                      type='checkbox'
                      checked={
                        selectedLoads.length === filteredLoads.length &&
                ...
                {filteredLoads.length === 0 ? (
                  ...
                  <div style={{ maxHeight: '500px', overflowY: 'auto' }}>
                    {filteredLoads.map((load) => {
                      const priority = loadPriorities.find(
                        (p) => p.id === load.id
                      );
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            transition: 'all 0.3s ease',
                            background: selectedLoads.includes(load.id)
                              ? '#f0f9ff'
                              : 'transparent',
                          }}
                          onMouseOver={(e) => {
                          ...
                          >
                              ...
                              <div style={{ flex: 1 }}>
                                ...
                                {workflowStatus && (
                                  ...
                                  >
                                    ...
                                    {workflowStatus.totalSteps} steps)
                            ...
                            >
                                    ...
                                    load.status === 'Available'
                                      ...
                                      : load.status === 'Assigned'
                                        ...
                                        : load.status === 'Broadcasted'
                                          ? '#e0e7ff'
                                          : load.status === 'Driver Selected'
                                            ? '#fed7aa'
                                            : load.status === 'Order Sent'
                                              ? '#bae6fd'
                              ...
                              {(load.dispatcherName ||
                                ...
                                >
                                  {load.dispatcherName && (
                                    <div>Dispatcher: {load.dispatcherName}</div>
                                  )}
                                  {load.assignedDriverName && (
                                    <div>Driver: {load.assignedDriverName}</div>
                                  )}
                                </div>
                              )}
                            </div>
              ...
              >
                ...
                {Object.keys(workflowStatuses).length === 0 ? (
                  ...
                  <div style={{ padding: '20px' }}>
                    {Object.entries(workflowStatuses).map(
                      ([loadId, workflow]) => (
                        <div
                          key={loadId}
                          style={{
                            ...
                            padding: '20px',
                            marginBottom: '16px',
                            border: workflow.requiresOverride
                              ? '2px solid #ef4444'
                              : '1px solid #e5e7eb',
                          }}
                        >
                          ...
                          >
                            ...
                            <div
                              style={{
                                background: workflow.requiresOverride
                                  ? '#fee2e2'
                                  : '#e0f2fe',
                                color: workflow.requiresOverride
                                  ? '#dc2626'
                                  : '#0369a1',
                                padding: '6px 12px',
                                borderRadius: '20px',
                          ...
                          <div style={{ marginBottom: '12px' }}>
                            ...
                            >
                              {workflow.completedSteps} of {workflow.totalSteps}{' '}
                              steps completed
                            </div>
                          </div>
                            ...
                            <button
                              onClick={() =>
                                addNotification(
                                  'info',
                                  'Override Processed',
                                  `Workflow override approved for load ${loadId}`
                                )
                              }
                              style={{
                                background: '#ef4444',
