...
export default function DispatchCentral() {
  ...
  const [selectedLoadForAssignment, setSelectedLoadForAssignment] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      ...
      const filteredLoads = userLoads.filter((load) => {
        const isAssignedToDispatcher =
          load.dispatcherId === currentUser.user.id ||
  ...
  const handleBulkBroadcast = () => {
    ...
    addNotification(
      'success',
      'Bulk Broadcast Complete',
      `${selectedLoads.length} loads broadcasted successfully`
    );
  };
  ...
  const handleBulkAssign = () => {
    ...
    addNotification(
      ...
      `${selectedLoads.length} loads assigned successfully`
    );
  };
  ...
  const handleDriverSelection = async (driver: Driver) => {
    ...
    const success = assignDispatcherToLoad(
      selectedLoadForAssignment,
      user.user.id
    );

    if (success) {
      // Find the load being assigned
      const assignedLoad = loads.find(
        (load) => load.id === selectedLoadForAssignment
      );
      ...
      if (assignedLoad) {
        ...
        try {
          const integrationResult =
            await LoadScheduleIntegrationService.integrateLoadAssignment({
              loadId: assignedLoad.id,
              loadBoardNumber: assignedLoad.loadBoardNumber,
              driverId: driver.id,
              driverName: driver.name,
              origin: assignedLoad.origin,
              destination: assignedLoad.destination,
              pickupDate: assignedLoad.pickupDate,
              deliveryDate: assignedLoad.deliveryDate,
              rate: assignedLoad.rate,
              distance: assignedLoad.distance,
              equipment: assignedLoad.equipment,
              weight: assignedLoad.weight,
              cargoWeight: assignedLoad.weight
                ? parseInt(assignedLoad.weight.replace(/[^0-9]/g, '')) || 0
                : 0,
              routeStates: extractStatesFromRoute(
                assignedLoad.origin,
                assignedLoad.destination
              ),
              specialInstructions: assignedLoad.specialInstructions,
              priority: (assignedLoad as any).priority || 'Medium',
              assignedBy: user.user.name || 'Dispatcher',
              customerId: (assignedLoad as any).customerId,
              brokerName: (assignedLoad as any).brokerName,
            });
          ...
          if (integrationResult.success) {
            ...
            // Handle weight compliance warnings
            if (
              ...
              addNotification(
                'warning',
                'Weight Compliance Warnings',
                integrationResult.warnings.join('. ')
              );
            }

            // Handle weight compliance information
            if (integrationResult.weightCompliance) {
              const compliance =
                integrationResult.weightCompliance.weightCompliance;
              if (!compliance.isCompliant) {
                const severity =
                  compliance.safetyRating === 'OVERWEIGHT'
                    ? 'error'
                    : 'warning';
                addNotification(
                  severity,
                  `Weight Compliance: ${compliance.safetyRating}`,
                  `${compliance.violations.length} violation(s) detected. ${compliance.requiredPermits.length > 0 ? 'Permits required: ' + compliance.requiredPermits.join(', ') : ''}`
                );
              } else {
                addNotification(
                  'info',
                  'Weight Compliance',
                  `✅ Load weight is compliant (${compliance.safetyRating})`
                );
              }
            }
            ...
            if (
              ...
              integrationResult.conflicts.length > 0
            ) {
              const conflictMessages = integrationResult.conflicts
            ...
            console.warn(
              '⚠️ Schedule integration failed:',
              integrationResult.error
            );
            addNotification(
              'warning',
              'Schedule Integration Issue',
              `Load assigned but schedule creation had issues: ${integrationResult.error}`
            );
          }
        } catch (error) {
          ...
          addNotification(
            ...
            'Load assigned successfully but could not create schedule entry'
          );
        }
      }
      ...
      addNotification(
        'success',
        'Driver Assigned',
        `${driver.name} assigned to load ${selectedLoadForAssignment} - Schedule created`
      );
    } else {
      addNotification(
        'error',
        'Assignment Failed',
        'Could not assign driver to load'
      );
    }
  ...
  const handleBroadcastLoad = (loadId: string) => {
    const updatedLoads = loads.map((load) =>
      load.id === loadId ? { ...load, status: 'Broadcasted' as any } : load
    );
    ...
    addNotification(
      'info',
      'Load Broadcasted',
      `Load ${loadId} has been broadcasted to all available drivers`
    );
  };
  ...
  const handleSetPriority = (
    ...
    addNotification(
      'warning',
      'Priority Set',
      `Load ${loadId} marked as ${priority} priority`
    );
  };

  // Helper function to extract states from route information
  const extractStatesFromRoute = (
    origin: string,
    destination: string
  ): string[] => {
    ...
    // Extract state abbreviations from origin and destination
    const routeText = `${origin} ${destination}`.toUpperCase();

    stateAbbreviations.forEach((state) => {
      if (routeText.includes(state)) {
        if (!extractedStates.includes(state)) {
          extractedStates.push(state);
        }
      }
    });

    // If no states found, default to FEDERAL for compliance checking
    return extractedStates.length > 0 ? extractedStates : ['FEDERAL'];
  };
  ...
  const filteredLoads = loads.filter((load) => {
    ...
    return (
      ...
      (load.assignedDriverName &&
        load.assignedDriverName.toLowerCase().includes(searchLower))
    );
  });
            ...
            <div>
              ...
              >
                <p style={{ fontSize: '16px', color: '#374151' }}>
                  Workflow monitoring system is active and ready to track load
                  progress.
                </p>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gap: '12px' }}>
              {mockDrivers.map((driver) => (
                <div
                  ...
                  onClick={() =>
                    driver.available && handleDriverSelection(driver)
                  }
                >
                  <div
                    style={{
