...
import DriverLoadPrioritizationPanel from '../components/DriverLoadPrioritizationPanel';
...
export default function DriverPortal() {
  ...
  const [notifications, setNotifications] =
    useState<SMSNotification[]>(MOCK_NOTIFICATIONS);
  const [loadConfirmations, setLoadConfirmations] = useState<
    LoadConfirmation[]
  >([]);
  const [deliveryVerifications, setDeliveryVerifications] = useState<
    DeliveryVerification[]
  >([]);
  const [activeTab, setActiveTab] = useState<
    'dashboard' | 'loads' | 'loadboard' | 'documents' | 'notifications'
  >('dashboard');
  ...
  const [workflows, setWorkflows] = useState<Map<string, LoadWorkflow>>(
    new Map()
  );
  const [activeWorkflowModal, setActiveWorkflowModal] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const assigned = allLoads.filter(
      (load) =>
        load.status === 'Assigned' &&
        load.dispatcherName === driver.dispatcherName
    );
    ...
    assigned.forEach((load) => {
      ...
      if (!workflow) {
        workflow = workflowManager.initializeLoadWorkflow(
          load.id,
          driver.id,
          'DSP-001'
        );
  ...
  const completeWorkflowStep = async (
    ...
    const submitBOLToWorkflow = async (loadId: string, workflowData: any) => {
      try {
        const load = assignedLoads.find((l) => l.id === loadId);
        if (!load) {
          throw new Error('Load not found');
        }

        // Generate proper load identifier
        const loadIdentifier =
          LoadIdentificationService.generateLoadIdentifiers({
            origin: load.origin,
        ...
        // Prepare BOL data from workflow
        const bolData = {
          bolNumber: `BOL-${Date.now().toString().slice(-6)}`,
          proNumber: `PRO-${Date.now().toString().slice(-6)}`,
          deliveryDate: workflowData.deliveryTimestamp
            ? new Date().toISOString().split('T')[0]
            : new Date().toISOString().split('T')[0],
          deliveryTime:
            workflowData.deliveryTimestamp ||
            new Date().toTimeString().split(' ')[0],
          receiverName: workflowData.receiverName || 'Unknown Receiver',
          receiverSignature: workflowData.receiverSignature || '',
          driverSignature: workflowData.signature || '',
          deliveryPhotos: workflowData.deliveryPhotos || [],
          pickupPhotos: workflowData.pickupPhotos || [],
          sealNumbers: workflowData.sealNumber ? [workflowData.sealNumber] : [],
          weight: workflowData.weight || load.weight || 'Unknown',
          pieces: workflowData.pieces || 1,
          damages: workflowData.damages || [],
          notes: workflowData.notes || '',
        };

        // Submit to BOL workflow
        const response = await fetch('/api/bol-workflow', {
          ...
          body: JSON.stringify({
            action: 'submit',
            loadId: load.id,
            loadIdentifierId: loadIdentifier.loadId,
            driverId: driver.id,
            driverName: driver.name,
            brokerId: load.brokerId || 'broker-js001', // Default broker
            brokerName: load.brokerName || 'John Smith',
            shipperId: load.shipperId || 'shipper-001',
            shipperName: load.shipperName || 'Unknown Shipper',
            shipperEmail: load.shipperEmail || 'shipper@company.com',
            bolData,
          }),
        });
        ...
        if (result.success) {
          console.log(`✅ BOL submitted to workflow: ${result.submissionId}`);
          console.log(`📧 Broker ${load.brokerName} notified for review`);
          alert(
            `✅ Delivery completed and BOL submitted!\n\nBOL ID: ${result.submissionId}\nBroker has been notified for review.`
          );
          return true;
        } else {
        ...
        alert(
          `❌ BOL submission failed: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
        return false;
      }
    };
    try {
      ...
      if (success) {
        ...
        // Submit BOL to workflow when delivery is completed
        if (stepId === 'delivery_completion') {
          await submitBOLToWorkflow(loadId, data);
        }
  ...
  const getWorkflowStepTitle = (stepId: string | null) => {
    ...
    return stepId
      ? titles[stepId as keyof typeof titles] || 'Unknown Step'
      : 'Unknown Step';
  };
  ...
  const getWorkflowStepDescription = (stepId: string | null) => {
    const descriptions = {
      load_assignment_confirmation:
        'Review and confirm receipt of this load assignment',
      rate_confirmation_review:
        'Review the rate confirmation document provided by dispatch',
      rate_confirmation_verification:
        'Verify and sign the rate confirmation details',
      bol_receipt_confirmation:
        'Confirm receipt of Bill of Lading from dispatcher',
      ...
      delivery_completion:
        'Complete delivery with receiver signature and documentation',
      pod_submission: 'Submit complete delivery documentation',
    };
    return stepId
      ? descriptions[stepId as keyof typeof descriptions] ||
    ...
    switch (stepId) {
      ...
      case 'delivery_completion':
        ...
        if (workflowStepData.overrideApproved) {
          ...
          return (
            basicValid &&
            workflowStepData.receiverName &&
            workflowStepData.receiverSignature
          );
        }
  ...
  const getBOLData = (loadId: string) => {
    ...
    workflow.steps.forEach((step) => {
      if (step.completed && step.data) {
        if (
          step.id === 'load_assignment_confirmation' ||
          step.id === 'pickup_completion'
        ) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        style={{ padding: '0 0 24px 0', maxWidth: '1400px', margin: '0 auto' }}
      >
        <div
          style={{
            display: 'flex',
          ...
          {[
            {
              ...
              icon: '📋',
              tooltip: 'Number of loads currently assigned to you',
            },
            {
              label: 'Hours Remaining',
              value: `${driver.hoursRemaining}h`,
              color: 'linear-gradient(135deg, #10b981, #059669)',
              icon: '⏰',
              tooltip: 'Hours remaining in your current duty cycle',
            },
            {
              ...
              icon: '📍',
              tooltip: 'Your current reported location',
            },
            {
              label: 'ELD Status',
              ...
              icon: '📡',
              tooltip: 'Electronic Logging Device connection status',
            },
            {
              ...
              icon: '💬',
              tooltip: 'Number of unread SMS notifications and messages',
            },
          ].map((stat, index) => (
        ...
        >
            ...
            <div>
              ...
              >
                ...
                >
                  ...
                  <div>
                    <strong>Assigned Truck:</strong> {driver.assignedTruck}
                    <br />
                    <strong>Current Location:</strong> {driver.currentLocation}
                    <br />
                    <strong>ELD Status:</strong>{' '}
                    <span
                      style={{
                        color:
                          driver.eldStatus === 'Connected'
              ...
              >
                ...
                >
                  <div>
                    <strong>Name:</strong> {driver.dispatcherName}
                    <br />
                    <strong>Phone:</strong> {driver.dispatcherPhone}
                    <br />
              ...
              >
                ...
                <div style={{ color: '#6b7280' }}>
                  • Load LD-2025-001 confirmed at 10:30 AM
                  <br />
                  • ELD sync completed at 8:00 AM
                  <br />
                  • Location updated: Dallas, TX at 9:15 AM
                  <br />• Dispatcher message received at 9:15 AM
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'loads' && (
            <div>
              ...
              {/* AI Load Prioritization Panel */}
              {assignedLoads.length > 0 && (
                <DriverLoadPrioritizationPanel
                  assignedLoads={assignedLoads.map((load) => ({
                    ...load,
                    workflowProgress: getWorkflowProgress(load.id),
                    nextStep: getNextWorkflowStep(load.id)?.name,
                  }))}
                  driverId={driver.id}
                />
              )}
                      ...
                      >
                          ...
                          <div style={{ marginBottom: '20px' }}>
                            ...
                            >
                              {workflow.steps.map((step, index) => {
                                ...
                                const canComplete = canCompleteWorkflowStep(
                                  load.id,
                        ...
                        >
                          {nextStep && (
                            ...
                            >
                              {getWorkflowStepIcon(nextStep.id, false, true)}{' '}
                              {nextStep.name}
                            </button>
                          )}
              ...
              >
                ...
                >
                  ...
                  <div style={{ color: '#6b7280', marginBottom: '16px' }}>
                    Download your rate confirmation documents for all assigned
                    loads.
                  </div>
                  <button
                    style={{
                ...
                >
                  ...
                  <div style={{ color: '#6b7280', marginBottom: '16px' }}>
                    Access your license, certifications, and employment
                    documents.
                  </div>
                  <button
                    style={{
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <div>
                  <strong>Load ID:</strong> {selectedLoad.id}
                </div>
                <div>
                  <strong>Rate:</strong> ${selectedLoad.rate.toLocaleString()}
                </div>
                <div>
                  <strong>Origin:</strong> {selectedLoad.origin}
                </div>
                <div>
                  <strong>Destination:</strong> {selectedLoad.destination}
                </div>
                <div>
                ...
                <div>
                  <strong>Equipment:</strong> {selectedLoad.equipment}
                </div>
              </div>
            </div>
            ...
            >
              ...
              <button
                onClick={() => {
                  if (
                    selectedLoad &&
                    (driverSignature || confirmationPhotos.length > 0)
                  ) {
                    ...
                    alert(
                      'Load confirmed successfully! You can now proceed to pickup.'
                    );
                  } else {
                    alert(
                      'Please provide either a signature or upload at least one photo to confirm the load.'
                    );
                  }
                }}
                style={{
              ...
              <button
                onClick={() => {
                  if (
                    ...
                    alert(
                      `Delivery completed successfully! Load ${selectedLoad.id} has been marked as delivered.`
                    );
                  } else {
                    alert(
                      'Please provide: receiver name, receiver signature, and at least one delivery photo to complete the delivery.'
                    );
                  }
                }}
                style={{
            ...
            <div>
              ...
              >
                ...
                <div style={{ fontSize: '14px', color: '#6b7280' }}>
                  ...
                  <strong>Route:</strong> {selectedLoad.origin} →{' '}
                ...
                <div style={{ marginBottom: '24px' }}>
                  ...
                  >
                    ...
                    <span style={{ fontSize: '14px', color: '#374151' }}>
                      I confirm receipt and acceptance of this load assignment{' '}
                  ...
                  >
                    ...
                    />
                    <span style={{ fontSize: '14px', color: '#374151' }}>
                      I accept the rate of ${selectedLoad.rate.toLocaleString()}{' '}
                      <span style={{ color: '#ef4444' }}>*</span>
                    </span>
                  </label>
                  ...
                  <div style={{ marginTop: '20px' }}>
                    ...
                    <SignaturePad
                      onSignatureChange={(signature) =>
                        setWorkflowStepData((prev) => ({ ...prev, signature }))
                      }
              ...
              {activeWorkflowModal === 'pickup_completion' && (
                <div style={{ marginBottom: '24px' }}>
                  ...
                  >
                    ...
                    >
                      You must upload a minimum of 2 photos: loaded truck and
                      bill of lading
                    </p>
                  </div>
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setWorkflowStepData((prev) => ({
                          ...prev,
                          loadingComplete: e.target.checked,
                        }))
                      }
                      style={{ marginRight: '8px', transform: 'scale(1.2)' }}
                    />
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setWorkflowStepData((prev) => ({
                          ...prev,
                          weightVerified: e.target.checked,
                        }))
                      }
                      style={{ marginRight: '8px', transform: 'scale(1.2)' }}
                    />
                  ...
                  <div style={{ marginBottom: '16px' }}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setWorkflowStepData((prev) => ({
                          ...prev,
                          pickupTimestamp: e.target.value,
                        }))
                      }
                      style={{
                        width: '100%',
                  ...
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    />
                    {workflowStepData.pickupPhotos &&
                      workflowStepData.pickupPhotos.length > 0 && (
                        <div style={{ fontSize: '12px', color: '#059669' }}>
                          ✅ {workflowStepData.pickupPhotos.length} photo(s)
                          uploaded
                          {workflowStepData.pickupPhotos.length >= 2
                            ? ' (Requirement met)'
                            : ' (Need minimum 2 photos)'}
                        </div>
                      )}
                    {uploadingFiles && (
                ...
                <div style={{ marginBottom: '24px' }}>
                  ...
                  >
                    ...
                    <p
                      style={{ fontSize: '14px', color: '#92400e', margin: 0 }}
                    >
                      You must upload minimum 2 photos and obtain receiver name
                      & signature (unless override approved)
                    </p>
                  </div>
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setWorkflowStepData((prev) => ({
                          ...prev,
                          unloadingComplete: e.target.checked,
                        }))
                      }
                      style={{ marginRight: '8px', transform: 'scale(1.2)' }}
                    />
                  ...
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    />
                    {workflowStepData.deliveryPhotos &&
                      workflowStepData.deliveryPhotos.length > 0 && (
                        <div style={{ fontSize: '12px', color: '#059669' }}>
                          ✅ {workflowStepData.deliveryPhotos.length} photo(s)
                          uploaded
                          {workflowStepData.deliveryPhotos.length >= 2
                            ? ' (Requirement met)'
                            : ' (Need minimum 2 photos)'}
                        </div>
                      )}
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setWorkflowStepData((prev) => ({
                            ...prev,
                            overrideApproved: e.target.checked,
                          }))
                        }
                        style={{ marginRight: '8px', transform: 'scale(1.2)' }}
                      />
                ...
                <button
                  onClick={() =>
                    completeWorkflowStep(
                      selectedLoad.id,
                      activeWorkflowModal as WorkflowStepId,
                      workflowStepData
                    )
                  }
                  ...
                  style={{
                    ...
                    fontSize: '14px',
                    fontWeight: '600',
                    cursor: isWorkflowStepValid(activeWorkflowModal)
                      ? 'pointer'
                      : 'not-allowed',
