...
// Comprehensive billing data structure - ORIGINAL DATA CLEARED
// Department-specific receivables data - RESTORED FROM GIT BUT DATA CLEARED
const mockDepartmentReceivables = [
  // ORIGINAL DATA COMMENTED OUT - HAD FULL FINANCIAL TRACKING:
  // Multiple departments: Broker Operations, Dispatch Operations, FreightFlow TMS, RFx, Insurance Partnerships
  // Full invoice data with commissions, audit trails, GL accounts
  // Real payment tracking and status management
  // Commission calculations and revenue recognition
].filter(() => false); // ORIGINAL DATA HAD 7+ INVOICES - NOW CLEARED

// BACKUP OF ORIGINAL STRUCTURE (commented out):
const originalMockDepartmentReceivables = [
  {
    id: 'DR-2025-001',
...
// Payroll data structure - ORIGINAL DATA CLEARED
const mockPayrollData = [
  // ORIGINAL PAYROLL DATA COMMENTED OUT - HAD FULL PAYROLL SYSTEM:
  // Employee details with commission tracking
  // Tax calculations and deductions
  // Direct deposit and banking integration
  // Audit trails and payroll processing
].filter(() => false); // ORIGINAL DATA HAD 4+ EMPLOYEES - NOW CLEARED

// BACKUP OF ORIGINAL PAYROLL STRUCTURE (commented out):
const originalMockPayrollData = [
  {
    id: 'PR-2025-001',
    employeeName: 'Mike Wilson',
    employeeId: 'MW-BB-2024061',
...
const mockAccountsReceivable = [
  // ORIGINAL A/R DATA COMMENTED OUT - HAD FULL RECEIVABLES TRACKING:
  // Invoice aging and payment tracking
  // Customer credit management
  // Collections and follow-up systems
].filter(() => false); // ORIGINAL DATA HAD 3+ A/R RECORDS - NOW CLEARED

// BACKUP OF ORIGINAL A/R STRUCTURE (commented out):
const originalMockAccountsReceivable = [
  {
    id: 'AR-2025-001',
    invoiceId: 'INV-2025-001',
    clientName: 'ABC Logistics Corp',
...
const mockAccountsPayable = [
  // ORIGINAL A/P DATA COMMENTED OUT - HAD FULL PAYABLES TRACKING:
  // Vendor management and payment scheduling
  // Purchase order tracking and approvals
  // Cash flow management and planning
].filter(() => false); // ORIGINAL DATA HAD 2+ A/P RECORDS - NOW CLEARED

// BACKUP OF ORIGINAL A/P STRUCTURE (commented out):
const originalMockAccountsPayable = [
  {
    id: 'AP-2025-001',
...
const BillingInvoicesPage = () => {
  ...
  // Generate chart data - ALL DATA CLEARED
  const getRevenueChartData = () => {
    // ORIGINAL: return departmentRevenue.map((dept) => ({
    //   label: dept.name.split(' ')[0],
    //   value: dept.totalRevenue,
    // }));
    return [
      { label: 'Broker', value: 0 }, // CLEARED FROM: dept.totalRevenue
      { label: 'Dispatch', value: 0 }, // CLEARED FROM: dept.totalRevenue
      { label: 'Freight', value: 0 }, // CLEARED FROM: dept.totalRevenue
      { label: 'Insurance', value: 0 }, // CLEARED FROM: dept.totalRevenue
    ];
  };
  ...
  const getCommissionChartData = () => {
    ...
    // }));
    return [
      { label: 'Broker', value: 0 }, // CLEARED FROM: dept.totalCommissions
      { label: 'Dispatch', value: 0 }, // CLEARED FROM: dept.totalCommissions
      { label: 'Freight', value: 0 }, // CLEARED FROM: dept.totalCommissions
      { label: 'Insurance', value: 0 }, // CLEARED FROM: dept.totalCommissions
    ];
  };

  const getTrendChartData = () => {
    // ORIGINAL: Mock trend data calculated from totalIncome
    // return [
    ...
    // ];
    return [
      { label: 'Week 1', value: 0 }, // CLEARED FROM: totalIncome * 0.2
      { label: 'Week 2', value: 0 }, // CLEARED FROM: totalIncome * 0.3
      { label: 'Week 3', value: 0 }, // CLEARED FROM: totalIncome * 0.25
      { label: 'Week 4', value: 0 }, // CLEARED FROM: totalIncome * 0.25
    ];
  };

  // Calculate performance metrics - DATA CLEARED
  const getPerformanceMetrics = () => {
    // ORIGINAL CALCULATIONS COMMENTED OUT - ALL DATA CLEARED
    // const revenuePerEmployee = totalDepartmentRevenue / (departmentRevenue.length || 1);
    // const avgDealSize = totalDepartmentRevenue / (dateFilteredDepartmentReceivables.length || 1);
    // etc...
      ...
      revenue: [
        {
          ...
          value: `$0`, // CLEARED FROM: ${revenuePerEmployee.toLocaleString()}
        },
        { label: 'Avg Deal Size', value: `$0` }, // CLEARED FROM: ${avgDealSize.toLocaleString()}
        { label: 'Conversion Rate', value: `0%` }, // CLEARED FROM: ${conversionRate.toFixed(1)}%
        {
          label: 'Collection Rate',
          value: `0%`, // CLEARED FROM: ${collectionEfficiency.toFixed(1)}%
        },
      ],
      efficiency: [
        {
          label: 'Profit Margin',
          value: `0%`, // CLEARED FROM: calculated profit margin
        },
        {
          label: 'Commission Rate',
          value: `0%`, // CLEARED FROM: calculated commission rate
        },
        {
          label: 'Payroll Efficiency',
          value: `0%`, // CLEARED FROM: calculated payroll efficiency
        },
        {
          label: 'Operating Ratio',
          value: `0%`, // CLEARED FROM: calculated operating ratio
        },
      ],
      growth: [
        { label: 'Revenue Growth', value: '0%' }, // CLEARED FROM: +12.5%
        { label: 'Client Growth', value: '0%' }, // CLEARED FROM: +8.3%
        { label: 'Efficiency Gain', value: '0%' }, // CLEARED FROM: +15.2%
        { label: 'Margin Improvement', value: '0%' }, // CLEARED FROM: +3.8%
      ],
    };
  };
        ...
        >
          ...
          <div style={{ color: 'white', fontSize: '0.95rem' }}>
            <div style={{ marginBottom: '6px' }}>
              ...
              {/* (paidReceivables + paidPayables).toLocaleString() */}0
            </div>
            <div style={{ marginBottom: '6px' }}>
              <strong>Pending:</strong> $
              {/* (pendingReceivables + pendingPayables).toLocaleString() */}0
            </div>
            <div style={{ color: '#22c55e' }}>
              <strong>Overdue:</strong> $
              {/* overdueReceivables.toLocaleString() */}0
            </div>
          </div>
        </div>
      </div>
          ...
          {costCenters.map((center) => {
            ...
            return (
              ...
              >
                ...
                >
                  Budget: ${/* center.budget.toLocaleString() */}0 | Spent: $
                  {/* center.spent.toLocaleString() */}0
                </div>
                <div
                  style={{
                    width: '100%',
    ...
    >
      ...
      {dateFilteredReceivables.map((item, index) => (
        ...
        >
          ...
          <div style={{ color: 'white' }}>{/* item.dueDate */}</div>
          <div style={{ color: 'white' }}>{/* item.aging */} days</div>
          <div style={{ display: 'flex', gap: '4px' }}>
            <button
              style={{
                padding: '6px 10px',
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                💰 Net Cash Flow: ${/* netCashFlow.toLocaleString() */}0 ($
                {formatDateRange()})
              </div>
              <div
                style={{
              ...
              >
                📊 Projected: ${/* projectedCashFlow.toLocaleString() */}0
              </div>
              <button
                ...
                {[
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        <div style={{ color: '#ef4444', fontWeight: '600' }}>
                          ${/* employee.commission.toLocaleString() */}0
                        </div>
                        <div
                          style={{
                            fontSize: '0.8rem',
