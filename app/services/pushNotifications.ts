...
class PushNotificationService {
      ...
      const subscription = await this.registration.pushManager.subscribe({
        ...
        applicationServerKey: this.urlB64ToUint8Array(
          process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY ||
            'BEl62iUYgUivxIkv69yViEuiBIa40HI80Y1nNHiHqhCBdOGNiTLgXXrmfzJyXdD6gvpKGXdMCsQRJ4U5-rNrVvE'
        ),
      });
  ...
  private isNotificationTypeEnabled(type: string): boolean {
    switch (type) {
      case 'workflow':
        return this.settings.workflowAlerts;
      case 'dispatch':
        return this.settings.dispatchMessages;
      case 'emergency':
        return this.settings.emergencyAlerts;
      case 'system':
        return this.settings.systemUpdates;
      case 'load_update':
        return this.settings.loadUpdates;
      default:
        return true;
    }
  }
