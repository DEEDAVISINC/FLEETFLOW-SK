...
export class FMCSAShipperIntelligenceService {
    ...
    try {
      ...
      for (const carrier of carriers) {
        ...
        shippers.forEach((shipper) => {
          ...
          if (existing) {
            shipperIntelligence.set(
              shipper.companyName,
              this.mergeShipperData(existing, shipper)
            );
          } else {
      ...
      const rankedShippers = await this.rankShipperProspects(
        Array.from(shipperIntelligence.values())
      );
   ...
   */
  private async analyzeCarrierForShippers(
    carrier: any
  ): Promise<ShipperIntelligence[]> {
    console.info(
      `🔍 Analyzing carrier ${carrier.carrierName} for shipper patterns...`
    );
    ...
    try {
      ...
      console.error(
        `❌ Error analyzing carrier ${carrier.carrierName}:`,
        error
      );
   ...
   */
  private async rankShipperProspects(
    shippers: ShipperIntelligence[]
  ): Promise<ShipperIntelligence[]> {
    ...
    try {
      ...
      return scoredShippers.sort(
        (a, b) => b.aiAnalysis.prospectScore - a.aiAnalysis.prospectScore
      );
    } catch (error) {
      console.error('❌ Error in AI prospect ranking:', error);
      return shippers.sort(
        (a, b) =>
          b.shippingPatterns.averageLoads - a.shippingPatterns.averageLoads
      );
    }
  }

  /**
   * Search for shippers by industry or criteria (simplified API)
   */
  async searchShippers(criteria: {
    ...
    console.info('🔍 Searching FMCSA for shippers:', criteria);

    try {
      // Use the main discovery method with adapted parameters
      const results = await this.discoverShippersFromCarriers({
        industryFocus: criteria.industry?.[0],
      ...
      if (criteria.minAnnualShipments) {
        filtered = filtered.filter(
          (s) => s.shippingPatterns.averageLoads >= criteria.minAnnualShipments
        );
      }

      console.info(`✅ Found ${filtered.length} matching shippers from FMCSA`);
      return filtered;
    } catch (error) {
      console.error('❌ Error searching FMCSA shippers:', error);
      return this.getMockShipperIntelligence({
        industryFocus: criteria.industry?.[0],
      });
    }
  }
   ...
   */
  async exportShippersToLeadGeneration(
    shippers: ShipperIntelligence[]
  ): Promise<{
    csvData: string;
    summary: {
    ...
    const csvRows = shippers.map((shipper) => [
      ...
      Math.round(
        shipper.associatedCarriers.reduce((avg, c) => avg + c.confidence, 0) /
          shipper.associatedCarriers.length
      ).toString(),
    ]);
    ...
    shippers.forEach((shipper) => {
      industryBreakdown[shipper.industryCategory] =
        (industryBreakdown[shipper.industryCategory] || 0) + 1;
    });
    ...
    return {
      ...
      summary: {
        ...
        highValueProspects: shippers.filter(
          (s) => s.aiAnalysis.prospectScore >= 80
        ).length,
        industryBreakdown,
        averageProspectScore: Math.round(
          shippers.reduce((sum, s) => sum + s.aiAnalysis.prospectScore, 0) /
            shippers.length
        ),
      },
    };
  ...
  private processAIAnalysisToShippers(
    aiAnalysis: any,
    carrier: any
  ): ShipperIntelligence[] {
    if (!aiAnalysis.identifiedShippers) return [];

    return aiAnalysis.identifiedShippers.map(
      (shipperData: any, index: number) => ({
  ...
  private mergeShipperData(
    existing: ShipperIntelligence,
    newData: ShipperIntelligence
  ): ShipperIntelligence {
    return {
      ...existing,
      associatedCarriers: [
        ...existing.associatedCarriers,
        ...newData.associatedCarriers,
      ],
      commonRoutes: [...existing.commonRoutes, ...newData.commonRoutes],
      shippingPatterns: {
        ...existing.shippingPatterns,
        averageLoads: Math.max(
          existing.shippingPatterns.averageLoads,
          newData.shippingPatterns.averageLoads
        ),
        equipmentTypes: [
          ...new Set([
            ...existing.shippingPatterns.equipmentTypes,
            ...newData.shippingPatterns.equipmentTypes,
          ]),
        ],
      },
      lastUpdated: new Date(),
  ...
  private getMockShipperIntelligence(
    searchCriteria: any
  ): ShipperIntelligence[] {
    return [
      {
        ...
        aiAnalysis: {
          ...
          recommendedApproach:
            'Direct outreach highlighting automotive expertise and regional coverage',
          competitiveLandscape: [
            'Regional Express (current)',
            'National carriers',
          ],
          bestContactTime: 'Tuesday-Thursday 10AM-2PM',
        },
