 ...
 * Enhanced with Brokerage Hierarchy Support (FBB -> BB relationship)
 */
...
export interface UserIdentificationData {
  ...
  phoneNumber?: string;
  location?: string;
  parentBrokerageId?: string; // For BB agents linked to FBB company
}
...
export class UserIdentificationService {
  ...
  private static readonly DEPARTMENT_CODES = {
    ...
    'Freight Brokerage': { code: 'FBB', color: 'orange' },
   ...
   * Example: FM-FBB-2024015 (Brokerage), ED-BB-2024061 (Agent)
   */
  static generateUserIdentifiers(
    userData: UserIdentificationData
  ): UserIdentifiers {
    const brokerInitials = this.generateBrokerInitials(
      userData.firstName,
      userData.lastName
    );
    const departmentInfo = this.getDepartmentInfo(
      userData.department,
      userData.role
    );
    ...
    const securityLevel =
      this.SECURITY_LEVELS[
        userData.role as keyof typeof this.SECURITY_LEVELS
      ] || 1;
   ...
   */
  private static generateBrokerInitials(
    firstName: string,
    lastName: string
  ): string {
    ...
    const baseInitials =
      `${safeFirstName.charAt(0)}${safeLastName.charAt(0)}`.toUpperCase();
   ...
   */
  private static getDepartmentInfo(
    department: string,
    role: string
  ): { code: string; color: string } {
    // Check department first
    let deptInfo =
      this.DEPARTMENT_CODES[department as keyof typeof this.DEPARTMENT_CODES];

    // If not found, check role
    if (!deptInfo) {
      deptInfo =
        this.DEPARTMENT_CODES[role as keyof typeof this.DEPARTMENT_CODES];
    }
  ...
  static getDepartmentColor(departmentCode: string): string {
    switch (departmentCode) {
      ...
      case 'FBB':
        return '#f97316'; // orange (same as BB for consistency)
      case 'DM':
  ...
  static getActiveBrokerInitials(): BrokerInitialsData[] {
    return [
      ...
      // Updated example: Broker Agent linked to Brokerage
      {
        initials: 'ED',
        firstName: 'Emily',
        lastName: 'Davis',
        ...
        parentBrokerageId: 'MW-FBB-2024046', // Links to Mike Wilson's brokerage
      },
      // Example: Freight Brokerage Business
      {
        initials: 'MW',
        firstName: 'Mike',
        lastName: 'Wilson',
        ...
        departmentColor: 'orange',
        isActive: true,
        assignedLoads: 0, // FBB manages agents, doesn't take individual loads
        specializations: ['Brokerage Management', 'Team Leadership'],
        region: 'Midwest',
  ...
  /**
   * Get agents for a brokerage (FBB -> BB relationship)
   */
  static getAgentsByBrokerageId(brokerageId: string): BrokerInitialsData[] {
    const brokers = this.getActiveBrokerInitials();
    return brokers.filter(
      (broker) =>
        broker.parentBrokerageId === brokerageId &&
        broker.departmentCode === 'BB'
    );
  }

  /**
   * Get brokerage for an agent (BB -> FBB relationship)
   */
  static getBrokerageByAgentId(agentId: string): BrokerInitialsData | null {
    const agent = this.getUserByBrokerInitials(agentId);
    if (!agent?.parentBrokerageId) return null;

    const brokers = this.getActiveBrokerInitials();
    return (
      brokers.find((broker) => broker.userId === agent.parentBrokerageId) ||
      null
    );
  }

  /**
   * Get department statistics with hierarchy info
   */
  static getDepartmentStats(): Record<
    string,
    { count: number; color: string; colorName: string }
  > {
    const brokers = this.getActiveBrokerInitials();
    const stats: Record<
      string,
   ...
   */
  static getDepartmentDisplayName(departmentCode: string): string {
    switch (departmentCode) {
      case 'DC':
        return 'Dispatcher';
      case 'BB':
