...
export interface NetworkLoad {
  ...
  isUrgent: boolean
  networkStatus: 'available' | 'bidding' | 'assigned' | 'in_transit' | 'completed'
  bids: NetworkBid[]
...
export class FreightNetworkService {
  ...
  private readonly PLATFORM_FEE_RATE = 0.04 // 4% commission

  // Load Management
  async postLoad(load: Omit<NetworkLoad, 'id' | 'bids' | 'createdAt' | 'updatedAt'>): Promise<NetworkLoad> {
  ...
  async submitBid(loadId: string, bid: Omit<NetworkBid, 'id' | 'status' | 'submittedAt'>): Promise<NetworkBid> {
  ...
  async acceptBid(loadId: string, bidId: string): Promise<NetworkTransaction> {
    ...
    const response = await fetch(`${this.API_BASE}/loads/${loadId}/bids/${bidId}/accept`, {
      method: 'POST'
    })
  ...
  // Capacity Sharing
  async postCapacity(capacity: Omit<NetworkCapacity, 'id' | 'createdAt'>): Promise<NetworkCapacity> {
  ...
  async invitePartner(email: string, message?: string): Promise<{ success: boolean; inviteId: string }> {
  ...
  async ratePartner(partnerId: string, rating: number, feedback?: string): Promise<void> {
    console.log('⭐ Rating network partner...')
    
    const response = await fetch(`${this.API_BASE}/partners/${partnerId}/rate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async findMatchingLoads(capacityId: string): Promise<NetworkLoad[]> {
    ...
    const response = await fetch(`${this.API_BASE}/capacity/${capacityId}/matches`)
  ...
  async getRevenueAnalytics(period: '7d' | '30d' | '90d' | '1y' = '30d'): Promise<{
    totalRevenue: number
    platformFees: number
    transactionCount: number
    averageTransactionValue: number
    revenueByDay: Array<{ date: string; revenue: number }>
    topPerformingRoutes: Array<{ route: string; revenue: number; count: number }>
  }> {
    console.log('💰 Fetching revenue analytics...')
    
    const response = await fetch(`${this.API_BASE}/analytics/revenue?period=${period}`)
