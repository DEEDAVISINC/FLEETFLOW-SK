...
class RealTimeTrackingService {
  ...
  private geofenceListeners: Array<(geofence: Geofence, entering: boolean) => void> = []
  ...
  private handleLocationUpdate(position: GeolocationPosition, driverId?: string) {
  ...
  async calculateRoute(origin: { lat: number; lng: number }, destination: { lat: number; lng: number }): Promise<RouteData | null> {
    try {
  ...
  private isLocationInGeofence(location: LocationData, geofence: Geofence): boolean {
  ...
  private handleGeofenceEvent(geofence: Geofence, entering: boolean) {
    console.log(`${entering ? 'Entering' : 'Exiting'} geofence: ${geofence.name}`)
  ...
  addGeofenceListener(listener: (geofence: Geofence, entering: boolean) => void) {
    this.geofenceListeners.push(listener)
  }

  removeGeofenceListener(listener: (geofence: Geofence, entering: boolean) => void) {
