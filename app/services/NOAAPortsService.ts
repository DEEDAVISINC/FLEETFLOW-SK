...
export class NOAAPortsService {
  ...
  private readonly NOAA_BASE_URL =
    'https://tidesandcurrents.noaa.gov/api/datagetter';
  private readonly NOAA_STATIONS_URL =
    'https://tidesandcurrents.noaa.gov/mdapi/prod/webapi/stations.json';
   ...
   */
  async getPortConditions(
    portCode: string
  ): Promise<NOAAPortConditions | null> {
  ...
  async getTidalPredictions(
    portCode: string,
    days: number = 7
  ): Promise<NOAATidalPrediction | null> {
    ...
    try {
      ...
      const predictions = await this.generateTidalPredictions(
        station.stationId,
        days
      );
   ...
   */
  async getMultiplePortConditions(
    portCodes: string[]
  ): Promise<NOAAPortConditions[]> {
  ...
  private async generateNOAAConditions(
    portCode: string,
    station: any
  ): Promise<NOAAPortConditions> {
    const now = new Date();
    ...
    const conditions: NOAAPortConditions = {
      ...
      conditions: this.generateOperationalConditions(
        windSpeed,
        currentSpeed,
        waterLevel
      ),
  ...
  private generateTidalPredictions(
    stationId: string,
    days: number
  ): NOAATidalPrediction {
    const predictions = [];
  ...
  private generateOperationalConditions(
    windSpeed: number,
    currentSpeed: number,
    waterLevel: number
  ) {
    let navigationStatus: 'optimal' | 'caution' | 'restricted' | 'closed' =
      'optimal';
  ...
  private assessNavigationSafety(conditions: NOAAPortConditions) {
    ...
    if (conditions.currents.speed > 2) {
      ...
      recommendations.push(
        'Account for strong currents in navigation planning'
      );
    }
  ...
  private getPortLocation(portCode: string) {
    ...
    return (
      locations[portCode as keyof typeof locations] || {
        lat: 0,
        lon: 0,
        state: 'US',
      }
