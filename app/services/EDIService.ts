...
export class EDIService {
   ...
   */
  static validateLoadForTransaction(
    loadData: any,
    transactionCode: string
  ): {
    isValid: boolean;
    ...
    if (!transaction) {
      return {
        isValid: false,
        missingRequired: [],
        invalidIdentifiers: ['Invalid transaction code'],
      };
    }
  ...
  static getTransactionType(
    loadData: any,
    context: 'status' | 'invoice' | 'manifest' | 'tender'
  ): string {
    switch (context) {
      case 'status':
  ...
  private static getIdentifierValue(
    ...
    const possibleFields = fieldMappings[identifier] || [
  ...
  static enrichLoadWithEDI(loadData: any, transactionContext?: string): any {
    ...
    if (transactionContext) {
      const validation = this.validateLoadForTransaction(
        enrichedLoad,
        transactionContext
      );
  ...
  static validateLoadIdentifier(identifier: string): boolean {
    const pattern =
      /^[A-Z]{2,4}-\d{3}-\d{3}-[A-Z]{2,4}-[A-Z]{2,4}-[A-Z]{2,4}-\d{3}-\d{8}$/;
    return pattern.test(identifier);
  }
  ...
  static generateLoadIdFromData(loadData: any): LoadIdentifier {
    ...
    const equipmentCode = this.getEquipmentTypeCode(
      loadData.equipmentType || 'dry van'
    );
    const priorityCode = this.getPriorityRateKey(
      loadData.priority || 'standard'
    );
  ...
  private static generateShipperSequence(
    ...
    const nextSequence =
      existingSequences.length > 0 ? existingSequences[0] + 1 : 1;
  ...
  static parseLoadBoardNumber(loadBoardNumber: string): {
    ...
    try {
      ...
      const brokerCode = parseInt(
        loadBoardNumber.substring(0, loadBoardNumber.length - 3)
      );
