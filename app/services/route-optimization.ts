...
export class RouteOptimizationService {
  ...
  private async runOptimizationAlgorithm(
    ...
    for (const vehicle of vehicles) {
      const route = await this.optimizeVehicleRoute(
        vehicle,
        sortedStops,
        matrix,
        constraints
  ...
  private async optimizeVehicleRoute(
    ...
    const maxTime = constraints.maxRouteTime
      ? constraints.maxRouteTime * 60
      : vehicle.maxDrivingHours * 60;
    ...
    while (availableStops.length > 0 && totalTime < maxTime) {
      ...
      for (let i = 0; i < availableStops.length; i++) {
        ...
        const distance = await this.getDirectDistance(
          currentLocation,
          stop.address
        );
    ...
    route.fuelCost =
      (route.totalDistance / vehicle.mpg) * this.getFuelPrice(vehicle.fuelType);
    route.tollCost = route.totalDistance * 0.05; // Estimate $0.05/mile in tolls
    route.estimatedCost =
      route.fuelCost + route.tollCost + (route.totalDuration / 60) * 25; // $25/hour driver cost
  ...
  private async addDetailedDirections(
    ...
    for (const route of routes) {
      ...
      try {
        ...
        if (data.status === 'OK' && data.routes.length > 0) {
          ...
          route.totalDistance = this.metersToMiles(
            googleRoute.legs.reduce(
          ...
          if (vehicle) {
            route.fuelCost =
              (route.totalDistance / vehicle.mpg) *
              this.getFuelPrice(vehicle.fuelType);
            route.estimatedCost =
              route.fuelCost + route.tollCost + (route.totalDuration / 60) * 25;
            route.efficiency = this.calculateEfficiencyScore(route, vehicle);
          }
        }
      } catch (error) {
  ...
  // Helper functions
  private async getDirectDistance(
    origin: string,
    destination: string
  ): Promise<number> {
  ...
  private getFuelPrice(fuelType: 'diesel' | 'gas'): number {
    return fuelType === 'diesel' ? 4.2 : 3.8; // Current average prices
  }
  ...
  private calculateEfficiencyScore(
    ...
    const distanceEfficiency = Math.max(
      0,
      100 - (route.totalDistance / 500) * 20
    ); // Penalty for long routes
    const timeEfficiency = Math.max(
      0,
      100 - (route.totalDuration / (vehicle.maxDrivingHours * 60)) * 30
    );
    const capacityEfficiency =
      (route.stops.reduce((sum, stop) => sum + (stop.weight || 0), 0) /
        vehicle.capacity) *
      100;

    return Math.round(
      (distanceEfficiency + timeEfficiency + capacityEfficiency) / 3
    );
  }
  ...
  private mockOptimization(
    ...
    vehicles.forEach((vehicle, index) => {
      ...
      const route: OptimizedRoute = {
        ...
        warnings:
          vehicleStops.length > 5
            ? ['High stop count may affect efficiency']
            : [],
      };
  ...
  calculateRouteMetrics(routes: OptimizedRoute[]) {
    ...
    const totalCost = routes.reduce(
      (sum, route) => sum + route.estimatedCost,
      0
    );
    const avgEfficiency =
