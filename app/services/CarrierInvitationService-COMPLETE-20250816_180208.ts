          ...
          <div style="padding: 40px; background: white;">
            ...
            <div style="background: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
              ...
              <ul style="color: #4b5563; line-height: 1.6;">
                ...
                <li>🤝 Professional broker relationships</li>
...
© 2024 FleetFlow
      `,
      variables: ['CONTACT_NAME', 'CARRIER_NAME', 'INVITER_NAME', 'COMPANY_NAME', 'INVITATION_LINK', 'EXPIRES_DATE', 'INVITER_EMAIL'],
      isDefault: true,
      createdBy: 'system',
    ...
    const defaultSMSTemplate: InvitationTemplate = {
      ...
      type: 'sms',
      htmlContent: '',
      textContent: `Hi {{CONTACT_NAME}}, {{INVITER_NAME}} from {{COMPANY_NAME}} invited {{CARRIER_NAME}} to join our FleetFlow carrier network. Quick onboarding: {{INVITATION_LINK}} (Expires {{EXPIRES_DATE}})`,
      variables: ['CONTACT_NAME', 'CARRIER_NAME', 'INVITER_NAME', 'COMPANY_NAME', 'INVITATION_LINK', 'EXPIRES_DATE'],
      isDefault: true,
      createdBy: 'system',
      createdDate: new Date().toISOString(),
    };
  ...
  public createInvitation(invitationData: Partial<CarrierInvitation>): CarrierInvitation {
    const invitation: CarrierInvitation = {
      ...
      expiresDate: invitationData.expiresDate || this.getDefaultExpirationDate(),
      ...
      metadata: {
        referralCode: this.generateReferralCode(invitationData.invitedBy!, invitationData.invitedByRole!),
    ...
    this.invitations.set(invitation.id, invitation);
    logger.info(`CarrierInvitationService: Created invitation ${invitation.id}`);
  ...
  private generateReferralCode(inviterName: string, role: string): string {
    const initials = inviterName.split(' ').map(n => n[0]).join('').toUpperCase();
  ...
  public updateInvitationStatus(invitationId: string, status: CarrierInvitation['status']): boolean {
    ...
    logger.info(`CarrierInvitationService: Updated invitation ${invitationId} status to ${status}`);
  ...
  public getInvitationsByStatus(status: CarrierInvitation['status']): CarrierInvitation[] {
  ...
  public getInvitationsByInviter(inviterName: string): CarrierInvitation[] {
    return this.getAllInvitations().filter(inv => inv.invitedBy === inviterName);
  }
  ...
  public getInvitationAnalytics(): InvitationAnalytics {
    ...
    const totalCompleted = allInvitations.filter(inv => inv.status === 'completed').length;

    // Calculate conversion rate
    const conversionRate = totalSent > 0 ? (totalCompleted / totalSent) * 100 : 0;

    // Calculate average time to complete (in hours)
    const completedInvitations = allInvitations.filter(inv => inv.completedDate && inv.sentDate);
    const averageTimeToComplete = completedInvitations.length > 0 
      ? completedInvitations.reduce((acc, inv) => {
        ...
        }, 0) / completedInvitations.length / (1000 * 60 * 60) // Convert to hours
      : 0;
  ...
  public getEmailTemplates(): InvitationTemplate[] {
    return Array.from(this.templates.values()).filter(t => t.type === 'email');
  }
  ...
  public processTemplate(template: InvitationTemplate, variables: Record<string, string>): {subject?: string, content: string} {
    let processedContent = template.type === 'email' ? template.htmlContent : template.textContent;
  ...
  public async sendInvitation(invitation: CarrierInvitation): Promise<{success: boolean, message: string}> {
    try {
      // In real implementation, this would integrate with email/SMS services
      logger.info(`CarrierInvitationService: Sending ${invitation.invitationType} invitation to ${invitation.targetCarrier.email}`);
      ...
      logger.error('CarrierInvitationService: Failed to send invitation', error);
      return {
        success: false,
  ...
  public createBulkInvitations(invitationRequests: Partial<CarrierInvitation>[]): CarrierInvitation[] {
    return invitationRequests.map(request => this.createInvitation(request));
  }

  // Get invitation by referral code
  public getInvitationByReferralCode(referralCode: string): CarrierInvitation | null {
    return this.getAllInvitations().find(inv => inv.metadata.referralCode === referralCode) || null;
  }
  ...
  public isInvitationValid(invitationId: string): boolean {
    ...
    return now <= expirationDate && invitation.status !== 'expired' && invitation.status !== 'declined';
  }
}
