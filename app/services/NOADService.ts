...
export interface NOADVesselData {
  ...
  // Vessel Information
  vesselName: string;
  vesselType: 'container' | 'bulk_carrier' | 'tanker' | 'roro' | 'general_cargo' | 'barge';
  mmsi: string;
  imoNumber: string;
  callSign: string;
  flagState: string;
  ...
  // Real-time Status
  currentStatus: 'scheduled' | 'arrived' | 'loading' | 'unloading' | 'departed' | 'delayed';
...
export class NOADService {
  ...
  private generateNOADVesselData(portCode?: string): NOADVesselData[] {
    const ports = [
      { code: 'USLAX', name: 'Port of Los Angeles', city: 'Los Angeles', state: 'CA', coords: [-118.2437, 34.0522] },
      { code: 'USNYK', name: 'Port of New York/New Jersey', city: 'New York', state: 'NY', coords: [-74.0060, 40.7128] },
      { code: 'USMIA', name: 'Port of Miami', city: 'Miami', state: 'FL', coords: [-80.1918, 25.7617] },
      { code: 'USSAV', name: 'Port of Savannah', city: 'Savannah', state: 'GA', coords: [-81.0912, 32.0835] },
      { code: 'USSEA', name: 'Port of Seattle', city: 'Seattle', state: 'WA', coords: [-122.3321, 47.6062] },
      { code: 'USCH1', name: 'Port of Charleston', city: 'Charleston', state: 'SC', coords: [-79.9311, 32.7765] },
      { code: 'USHOU', name: 'Port of Houston', city: 'Houston', state: 'TX', coords: [-95.3698, 29.7604] },
    ];

    const vessels = [
      { name: 'MSC GÜLSÜN', type: 'container', mmsi: '636019825', imo: '9811000', callSign: 'A8OW8', flag: 'LR' },
      { name: 'EVER ACE', type: 'container', mmsi: '416001234', imo: '9811001', callSign: 'VRPZ6', flag: 'PA' },
      { name: 'COSCO SHIPPING UNIVERSE', type: 'container', mmsi: '477123456', imo: '9811002', callSign: 'BQVP', flag: 'HK' },
      { name: 'ATLANTIC HARMONY', type: 'bulk_carrier', mmsi: '538123456', imo: '9811003', callSign: 'V7A123', flag: 'MH' },
      { name: 'PACIFIC PIONEER', type: 'tanker', mmsi: '636987654', imo: '9811004', callSign: 'A8ZZ9', flag: 'LR' },
      { name: 'CARGO EXPRESS', type: 'general_cargo', mmsi: '367123456', imo: '9811005', callSign: 'WDF123', flag: 'US' }
    ];

    const commodities = ['Electronics', 'Automobiles', 'Machinery', 'Chemicals', 'Food Products', 'Textiles', 'Steel', 'Petroleum Products'];
    const statuses = ['scheduled', 'arrived', 'loading', 'unloading', 'departed', 'delayed'];
    ...
    filteredPorts.forEach(port => {
      ...
      for (let i = 0; i < vesselCount; i++) {
        ...
        arrivalTime.setHours(arrivalTime.getHours() + Math.floor(Math.random() * 48) - 24);
        
        const departureTime = new Date(arrivalTime);
        departureTime.setHours(departureTime.getHours() + Math.floor(Math.random() * 36) + 6);
        ...
        vesselData.push({
          ...
          cargoManifest: [{
            ...
            containerCount: vessel.type === 'container' ? Math.floor(Math.random() * 2000) + 500 : undefined
          }],
          ...
          delayReasons: status === 'delayed' ? ['Port Congestion', 'Weather'] : undefined,
  ...
  private generatePortIntelligence(portCode?: string): PortIntelligence {
    ...
    const port = portCode ? ports.find(p => p.code === portCode) || ports[0] : ports[0];
    const congestionLevels = ['low', 'medium', 'high', 'critical'];
    const commodities = ['Electronics', 'Automobiles', 'Chemicals', 'Food Products', 'Steel'];
    ...
    return {
      ...
      congestionLevel: congestionLevels[Math.floor(Math.random() * congestionLevels.length)] as any,
      bottleneckReasons: ['High cargo volume', 'Limited berth availability', 'Weather conditions'],
  ...
  private generateVesselSchedules(portCode?: string): VesselSchedule[] {
    const vessels = [
      ...
      { name: 'ATLANTIC HARMONY', type: 'Bulk Carrier', line: 'Atlantic Shipping' }
    ];
    ...
    vessels.forEach(vessel => {
      ...
      departure.setHours(departure.getHours() + Math.floor(Math.random() * 24) + 6);
